...
Current candidate: PR884 graphical-export follow-up; the exact final revision and CI run are recorded in the PR evidence after the consolidated repair is pushed.
...
| Unified editable WBS, Kanban, timeline, Gantt and monthly calendar | `ProjectFlowHivePsaWorkspace.jsx`, `ProjectFlowHiveCenter.jsx`, `project-forge/ProjectForgeViews.jsx`, `project-flowhive-planner-layout.css`, `ProjectFlowHivePsaArtifactRenderer.cs` | Prior PR884 graphical checks passed; this follow-up adds complete schedule-row retention, overflow markers and a paginated task-details appendix. Final FlowHive CI run and rendered evidence for the exact follow-up head are recorded in PR884 evidence | None; PR884 follow-up is not deployed | Not run; CI fixtures are not live UAT | Prove one authoritative editable task graph across all views, including persistence, accessibility, concurrent edits and live schedule behavior |
| Canonical task adoption and FlowHive/Project Forge consistency | `PostgresProjectFlowHivePlanRepository.cs`, `ProjectForgeModule.cs`, `ProjectForgeFlowHiveSyncPortal.jsx`, `ProjectForgeInteractiveModule.cs`, `ProjectFlowHivePlanningContracts.cs`, `ProjectFlowHiveFinancialReadback.cs` | Existing adoption transaction locks the project, checks the reviewed plan revision, links stable plan-task/canonical-task identities and rejects repeat adoption without creating duplicates; isolated PostgreSQL execution tests now cover deterministic canonical financial readback and replay idempotency | None; candidate only | Not run | Prove stable planning IDs map idempotently to canonical tasks and assignments without duplicating tasks or time records in live UAT |
| Authoritative project hours and forecasts | `ProjectFlowHiveFinancialReadback.cs`, `ProjectForgeModule.cs`, `ProjectFinancialTruthModule.cs`, `FinancialOperationsContracts.cs`, `FinancialOperationsSourceLoader.cs`, `UnifiedProjectFinancialWorkspace.jsx` | Isolated PostgreSQL execution tests cover approved-time status filtering, task-rate cost, budget remaining, estimated effort remaining, recorded forecast authority and unknown-rate behavior; final CI run is recorded in PR884 evidence | None; candidate only | Not run | Reconcile approved budget, actual time, estimate-to-complete and forecast with authoritative timesheets; restricted values must remain unknown, not zero |
...
| Six US Signal-branded XLSX/PDF exports | `ProjectFlowHivePsaArtifactRenderer.cs`, `ProjectFlowHiveArtifactRenderer.cs`, `AnalyticsBrandedExportBuilder.cs`, `Module025SowGsdDocumentExporter.cs` | PR883 run `34276704342` passed exact head `f7f03e3fb263e1ce26e8401c4ae811a9f80fb174` with Unicode PDF extraction/rendering and lossless typed XLSX checks; PR884 follow-up adds eight-overlap, cross-month, invalid/reversed-date, long-label, multi-page and ID-reconciliation coverage plus rendered-page evidence. Final CI run and artifact path are recorded in PR884 evidence | None; PR884 follow-up is not deployed | Not run; synthetic artifacts are not live customer/internal delivery evidence | Complete deployed UAT export review for all twelve artifacts, including authorized downloads, typed data/formula safety, pagination, branding, graphical layouts and redaction |
