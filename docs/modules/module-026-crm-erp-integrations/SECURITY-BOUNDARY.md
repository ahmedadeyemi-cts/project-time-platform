...
- ConnectWise SELL record imports are response-size bounded and retain only configured mapped fields; raw responses are not stored.
- ConnectWise SELL lookup templates require a literal `{recordId}` token and pass the same public-HTTPS/private-address protections.
