...
| Read-only test | `GET /api/quotes?page=1&pageSize=1&includeFields=id` |
...
required, because the old contacts API belongs to another vendor. Quote and pricing mappings are deliberately empty until the tenant contract is verified. Separate quote items need a reviewed labor-rate adapter before work intake can create records. Document
