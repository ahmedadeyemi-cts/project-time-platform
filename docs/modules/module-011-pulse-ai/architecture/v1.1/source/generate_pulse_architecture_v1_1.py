#!/usr/bin/env python3
"""Generate the canonical US Signal Pulse AI Architecture Package v1.1.
...
VERSION = "1.1"
DOC_TITLE = "Pulse AI Private Intelligence Architecture"
...
FILES = {
    "docx": "US_Signal_Pulse_AI_Private_Intelligence_Architecture_v1.1.docx",
    "pdf": "US_Signal_Pulse_AI_Private_Intelligence_Architecture_v1.1.pdf",
    "diagrams_pdf": "US_Signal_Pulse_AI_Architecture_Diagrams_v1.1.pdf",
    "logical_png": "US_Signal_Pulse_AI_Logical_Architecture_v1.1.png",
    "logical_svg": "US_Signal_Pulse_AI_Logical_Architecture_v1.1.svg",
    "deployment_png": "US_Signal_Pulse_AI_Deployment_Network_Architecture_v1.1.png",
    "deployment_svg": "US_Signal_Pulse_AI_Deployment_Network_Architecture_v1.1.svg",
}

SECTIONS = [
    ("1. Executive Summary", [
        "Pulse AI is the private, permission-aware intelligence layer for Pulse. It combines authorized internal documents, governed live system data, deterministic calculations, and a private language model so users receive detailed, source-grounded answers and reviewable drafts without exposing confidential information to public external models.",
        "Every request begins with authentication and authorization. Pulse retrieves only the document sections and live records the effective user is allowed to access. The private Pulse AI model reasons over that context, produces a structured result, and measures confidence, source coverage, freshness, conflicts, and calculation validity.",
    ...
    ("5. Component Architecture", [
        "Pulse experience layer: Timesheet, Ask Pulse AI, global Search, FlowHive, Reporting, Financial Insights, and future module surfaces.",
    ...
    ("8. Use Case Architecture", [
        "Timesheet: the Engineer’s rough note remains the primary statement of work. Pulse retrieves authorized SOW, GSD, task, request, and engineering documents, drafts a description, and requires the Engineer to review and apply it. Pulse AI cannot change hours, save, submit, or approve time.",
    ...
    ("10. Governed Live-Data Tools and Semantic Layer", [
        "Pulse AI never receives unrestricted database credentials. Information is exposed through allowlisted read-only APIs and semantic metric contracts owned by the applicable Pulse module.",
    ...
    ("18. Self-Sustaining Operating Model", [
        "Pulse AI may automatically detect approved document changes, refresh indexes, revoke inaccessible content, monitor source freshness, capture feedback, run scheduled evaluations, detect quality drift, and prepare sanitized training candidates.",
        ...
        "Pulse AI never autonomously modifies its own policies, tools, prompts, permissions, training data, model weights, deployment, or production route.",
    ]),
...
GLOSSARY = [
    ...
    ["Pulse AI", "Module 011 private intelligence, knowledge, evaluation, and model-lifecycle capability."],
...
def svg_header(width: int, height: int, title: str) -> list[str]:
    return [
        ...
        f'<text x="220" y="91" class="sub">US Signal • Pulse • Module 011 • Version {VERSION}</text>',
...
def build_logical_svg() -> str:
    w, h = 1600, 1050
    s = svg_header(w, h, "Pulse AI Private-First Logical Architecture")
    ...
    s += svg_box(75,200,250,90,"Pulse User Experience",["Timesheet • Ask Pulse AI • Search","FlowHive • Reports • Financials"],tag="ENTRY")
    s += svg_box(390,200,300,90,"Identity and Authorization",["Session • actual/effective user","RBAC • module • project • record scope"])
    s += svg_box(760,200,300,90,"Pulse AI Orchestrator",["Intent • policy • tool plan","minimum-necessary context assembly"])
    ...
    s += svg_box(785,385,300,120,"Private Pulse AI Model",["open-weight private endpoint","structured deep-response schemas","no public inbound access"],fill="#EDF8FE",stroke=BLUE,tag="PRIMARY")
...
def build_deployment_svg() -> str:
    w, h = 1800, 1100
    s = svg_header(w, h, "Pulse AI Deployment and Network Architecture")
...
def configure_docx(doc: Document):
    ...
    for section in doc.sections:
        ...
        hp=section.header.paragraphs[0]; hp.alignment=WD_ALIGN_PARAGRAPH.RIGHT; r=hp.add_run(f"US Signal  |  Pulse AI Architecture  |  v{VERSION}"); r.font.name="Arial"; r.font.size=Pt(7.5); r.font.color.rgb=RGBColor.from_string(MID_BLUE.replace("#",""))
...
def build_docx():
    ...
    p=doc.add_paragraph("Private-first document intelligence, governed live-data reasoning, and controlled external LLM escalation"); p.alignment=WD_ALIGN_PARAGRAPH.CENTER; p.runs[0].font.size=Pt(13); p.runs[0].font.color.rgb=RGBColor.from_string(DARK_GRAY.replace('#',''))
    doc.add_paragraph(); add_docx_table(doc,[["Document owner","Product","Version","Classification"],[OWNER,PRODUCT,VERSION,CLASSIFICATION],["Architecture status","Module","Prepared for","Canonical name"],["Review baseline","011 — Pulse AI","US Signal architecture review","Pulse"]])
    ...
    for title,paragraphs in SECTIONS:
        ...
        if title.startswith("4."): doc.add_paragraph("Figure 1. Pulse AI private-first logical architecture and confidence-driven external escalation path."); doc.add_picture(str(OUT/FILES["logical_png"]),width=Inches(7.0))
        if title.startswith("14."): doc.add_paragraph("Figure 2. Pulse AI deployment zones, private data paths, governed tools, controlled egress, and audit integration."); doc.add_picture(str(OUT/FILES["deployment_png"]),width=Inches(7.0))
        ...
        if title.startswith("5."): add_docx_table(doc,[["Layer","Key components","Authority"],["Experience","Timesheet, Ask Pulse AI, Search, FlowHive, Reports","Owning Pulse modules"],["Policy","Identity, RBAC, scope, classification","Pulse backend"],["Knowledge","Storage, extraction, embeddings, search","Private data services"],["Tools","Read-only APIs and semantic metrics","Owning modules"],["Reasoning","Private model, confidence, verifier","Module 011"],["Egress","DLP, Module 064, external providers","Security policy + Module 064"]])
...
def build_pdf():
    ...
    story += [Spacer(1,.35*inch),RLImage(str(LOGO),width=1.55*inch,height=1.0*inch),Spacer(1,.1*inch),Paragraph("US SIGNAL CREATED ARCHITECTURE DOCUMENT",ParagraphStyle("eye",parent=st["small"],fontName="Helvetica-Bold",fontSize=10,textColor=colors.HexColor(BLUE),alignment=TA_CENTER)),Spacer(1,.15*inch),Paragraph(DOC_TITLE,st["title"]),Paragraph("Private-first document intelligence, governed live-data reasoning, and controlled external LLM escalation",st["subtitle"]),Spacer(1,.2*inch),rl_table([["Document owner","Product","Version","Classification"],[OWNER,PRODUCT,VERSION,CLASSIFICATION],["Architecture status","Module","Prepared for","Canonical name"],["Review baseline","011 — Pulse AI","US Signal architecture review","Pulse"]],[1.25*inch,1.05*inch,.7*inch,3.7*inch]),Spacer(1,.25*inch),Paragraph("This architecture package is created and owned by US Signal and uses the exact supplied US Signal logo.",st["subtitle"]),PageBreak()]
    story += [Paragraph("Document Control",st["h1"]),rl_table([["Field","Value"],["Title",DOC_TITLE],["Owner",OWNER],["Product",PRODUCT],["Module","011 — Pulse AI"],["Version",VERSION],["Status","Review baseline"],["Classification",CLASSIFICATION]],[1.4*inch,5.8*inch]),PageBreak(),Paragraph("Contents",st["h1"])]
    ...
    for title,paragraphs in SECTIONS:
        ...
        if title.startswith("4."): story += [RLImage(str(OUT/FILES["logical_png"]),width=7*inch,height=3.05*inch),Paragraph("Figure 1. Pulse AI private-first logical architecture.",st["caption"])]
        if title.startswith("14."): story += [RLImage(str(OUT/FILES["deployment_png"]),width=7*inch,height=3.15*inch),Paragraph("Figure 2. Pulse AI deployment and network architecture.",st["caption"])]
        ...
        if title.startswith("5."): story.append(rl_table([["Layer","Key components","Authority"],["Experience","Timesheet, Ask Pulse AI, Search, FlowHive, Reports","Owning modules"],["Policy","Identity, RBAC, scope, classification","Pulse backend"],["Knowledge","Storage, extraction, embeddings, search","Private data services"],["Tools","Read-only APIs and metrics","Owning modules"],["Reasoning","Private model, confidence, verifier","Module 011"],["Egress","DLP, Module 064, external providers","Security + Module 064"]],[1*inch,3.3*inch,2.9*inch]))
...
def build_diagram_pdf():
    ...
    for title,filename in [("Pulse AI Private-First Logical Architecture",FILES["logical_png"]),("Pulse AI Deployment and Network Architecture",FILES["deployment_png"])]:
...
def main():
    ...
    print(f"PULSE_AI_ARCHITECTURE_PACKAGE_VERSION={VERSION}"); print(f"PULSE_AI_ARCHITECTURE_FILES={len(hashes)}")
