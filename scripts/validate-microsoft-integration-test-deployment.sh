...
require "$WORKFLOW" 'Validate served Microsoft Integration assets and ready revision images'
require "$WORKFLOW" 'properties.latestReadyRevisionName'
require "$WORKFLOW" 'az containerapp revision show'
...
require "$APPLY" 'MICROSOFT_INTEGRATION_EVIDENCE_BASELINE'
require "$APPLY" 'SECRET_ROWS_BEFORE'
require "$APPLY" 'AUDIT_ROWS_BEFORE'
require "$APPLY" 'Migration 045 changed existing Microsoft Integration secret or audit evidence counts.'
...
reject "$APPLY" 'Migration 045 unexpectedly created secret or audit records.'
