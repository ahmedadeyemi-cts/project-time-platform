...
MIGRATION_IMAGE="${PROJECTPULSE_PR479_MIGRATION_IMAGE:-}"
DATABASE_URL="${PROJECTPULSE_TEST_DATABASE_URL:-}"
JOB_NAME="${PROJECTPULSE_PR479_MIGRATION_JOB_NAME:-}"
LOG_DIR="${PROJECTPULSE_PR479_MIGRATION_LOG_DIR:-}"
...
[[ -n "$MIGRATION_IMAGE" ]] || fail "PROJECTPULSE_PR479_MIGRATION_IMAGE is not configured."
...
[[ "$JOB_NAME" =~ ^[a-z][a-z0-9-]{0,30}[a-z0-9]$ ]] ||
  fail "PROJECTPULSE_PR479_MIGRATION_JOB_NAME is invalid."
[[ "$MIGRATION_IMAGE" == "$ACR_NAME.azurecr.io/"*@sha256:* ]] ||
  fail "Migration image must be an immutable approved-ACR digest."
if [[ "$MODE" == rollback ]]; then
  [[ "$ROLLBACK_CONFIRMATION" == "ROLLBACK-MIGRATIONS-068-070-3471962B" ]] ||
...
cleanup() {
  ...
  if (( JOB_CREATED == 1 )); then
    ...
    echo "PROJECTPULSE_PR479_MIGRATION_JOB_CLEANUP=COMPLETE"
  fi
  exit "$status"
}
...
env_args=(
  PROJECTPULSE_TEST_DATABASE_URL=secretref:pr479-db-url
  PROJECTPULSE_PR479_MIGRATION_MODE="$MODE"
  PROJECTPULSE_PR479_NEW_068="$NEW_068"
  PROJECTPULSE_PR479_NEW_069="$NEW_069"
  PROJECTPULSE_PR479_NEW_070="$NEW_070"
)
if [[ "$MODE" == rollback ]]; then
  env_args+=(PROJECTPULSE_PR479_ROLLBACK_CONFIRMATION="$ROLLBACK_CONFIRMATION")
fi
...
az containerapp job create \
  ...
  --memory 0.5Gi \
  "${identity_args[@]}" \
  --secrets "pr479-db-url=$DATABASE_URL" \
  --env-vars "${env_args[@]}" \
  --tags \
    projectpulse-scope="migrations-068-070-test" \
...
echo "PROJECTPULSE_PR479_MIGRATION_JOB_CREATE=COMPLETE mode=$MODE"
...
echo "PROJECTPULSE_PR479_MIGRATION_JOB_EXECUTION=$EXECUTION_NAME"
...
for attempt in $(seq 1 180); do
  ...
  case "$STATUS" in
    Succeeded)
      ...
      echo "PROJECTPULSE_PR479_MIGRATION_JOB_STATUS=SUCCEEDED mode=$MODE"
      exit 0
      ;;
    Failed|Stopped|Canceled|Degraded)
      emit_logs
      echo "PROJECTPULSE_PR479_MIGRATION_JOB_STATUS=$STATUS mode=$MODE" >&2
      break
      ;;
  esac
  sleep 5
...
fail "The private-network release migration job did not succeed in $MODE mode."
