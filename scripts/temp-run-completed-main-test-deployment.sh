...
cleanup() {
  ...
  if (( status != 0 )); then
    set +e
    rollback_app WEB "$WEB_APP" "$WEB_IMAGE" "$CURRENT_WEB_IMAGE" "$WEB_STARTED" cmweb-rb
    rollback_app API "$API_APP" "$API_IMAGE" "$CURRENT_API_IMAGE" "$API_STARTED" cmapi-rb
    set -e
  fi
  exit "$status"
}
...
probe_json() {
  ...
  for attempt in $(seq 1 24); do
    ...
    if [[ ",$expected," == *",$status,"* && "$type" == application/json* ]]; then
      ...
      echo "CUMULATIVE_PROBE_${name^^}=HTTP_$status"
      return 0
    fi
    sleep 5
  done
...
probe_version_commit() {
  ...
  echo "CUMULATIVE_VERSION_ENDPOINT=PASS"
}
...
PY
  ...
  echo "CUMULATIVE_SSO_STATE_ORIGIN_RECOVERY=HTTP_$callback_status"
}
...
bundle_markers() {
  ...
  for marker in \
    ...
    'Billing Readiness & Reconciliation Recovery' \
  ...
  for marker in \
    ...
    '.pulse-ai-doc-workbench' \
    '.pulse-ai-deep-workbench'; do
    grep -Fq "$marker" "$css" || fail "Missing served CSS marker: $marker"
  done
  echo 'CUMULATIVE_SERVED_BUNDLE=PASS'
}
...
API_REVISION="$API_APP--cmapi-${RUN_ID}-${RUN_ATTEMPT}"
API_STARTED=1
az containerapp update -g "$RESOURCE_GROUP" -n "$API_APP" \
  ...
  --revision-suffix "cmapi-${RUN_ID}-${RUN_ATTEMPT}" \
...
probe_version_commit
probe_json g4_routing '/api/project-notifications/routing-rules' '401,403'
probe_json g4_schedules '/api/project-notifications/schedules' '401,403'
probe_json g4_monitor '/api/project-notifications/delivery-monitor' '401,403'
probe_json g4_readiness '/api/project-notifications/module-065-readiness' '401,403'
probe_json g5_catalog '/api/financial-operations/reports/catalog' '401,403'
probe_json g5_sources '/api/financial-operations/sources' '401,403'
probe_json g5_workbench '/api/financial-operations/workbench' '401,403'
probe_json g5_closeout '/api/financial-operations/modules/041' '401,403'
probe_json g5_billing '/api/financial-operations/modules/042' '401,403'
probe_json pulse_overview '/api/pulse-ai/v1/overview' '401,403'
probe_json pulse_private_runtime '/api/pulse-ai/v1/private-runtime/readiness' '401,403'
probe_json pulse_doc_pipeline '/api/pulse-ai/v1/documents/pipeline/readiness' '401,403'
probe_json pulse_doc_inventory '/api/pulse-ai/v1/documents/inventory' '401,403'
probe_sso_state_origin_recovery

WEB_REVISION="$WEB_APP--cmweb-${RUN_ID}-${RUN_ATTEMPT}"
WEB_STARTED=1
az containerapp update -g "$RESOURCE_GROUP" -n "$WEB_APP" \
  --image "$WEB_IMAGE" \
  --revision-suffix "cmweb-${RUN_ID}-${RUN_ATTEMPT}" \
  --output none --only-show-errors
bash "$WAIT_SCRIPT" "$RESOURCE_GROUP" "$WEB_APP" "$WEB_REVISION" "$WEB_IMAGE" 60 10
probe_json proxy_g5 '/api/financial-operations/reports/catalog' '401,403'
probe_json proxy_pulse '/api/pulse-ai/v1/documents/pipeline/readiness' '401,403'
bundle_markers
...
{
  "environment": "test",
  "deploymentType": "cumulative-api-web-after-migrations-050-051",
  ...
  "migration050Verified": true,
  "migration051Verified": true,
  "group4Included": true,
  "group5Included": true,
  "pulseAiDeepIntelligenceIncluded": true,
  "pulseAiPrivateDocumentPipelineIncluded": true,
  "module038RegressionOnly": true,
...
echo 'COMPLETED_MAIN_TEST_DEPLOYMENT=COMPLETE'
