...
require "$WORKFLOW" '$BASE/api/runtime/v2/readiness'
require "$WORKFLOW" '.status == "combined_module_runtime_ready"'
require "$WORKFLOW" '.roleCount == 12'
require "$WORKFLOW" '.moduleCount == 70'
require "$WORKFLOW" '.foundationalMigrationCount == 2'
require "$WORKFLOW" '.expenseMigrationCount == 2'
require "$WORKFLOW" '.expenseTableCount == 6'
require "$WORKFLOW" '/api/project-expenses/readiness'
require "$WORKFLOW" '.status == "project_expense_runtime_ready"'
require "$WORKFLOW" '.permissionCount == 7'
