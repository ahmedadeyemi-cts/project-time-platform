...
cleanup() {
  ...
  if (( status != 0 )); then
    set +e
    rollback_app WEB "$WEB_APP" "$WEB_IMAGE" "$CURRENT_WEB_IMAGE" "$WEB_STARTED" cmainweb-rb
    rollback_app API "$API_APP" "$API_IMAGE" "$CURRENT_API_IMAGE" "$API_STARTED" cmainapi-rb
    set -e
  fi
  exit "$status"
}
...
probe_json() {
  ...
  for attempt in $(seq 1 24); do
    ...
    if [[ ",$expected," == *",$status,"* && "$type" == application/json* ]]; then
      ...
      echo "CURRENT_MAIN_PROBE_${name^^}=HTTP_$status"
      return 0
    fi
    sleep 5
  done
...
probe_version_commit() {
  ...
  echo "CURRENT_MAIN_VERSION_ENDPOINT=PASS"
}
...
PY
  ...
  echo "CURRENT_MAIN_SSO_STATE_ORIGIN_RECOVERY=HTTP_$callback_status"
}
...
API_REVISION="$API_APP--cmainapi-${RUN_ID}-${RUN_ATTEMPT}"
API_STARTED=1
az containerapp update -g "$RESOURCE_GROUP" -n "$API_APP" \
  ...
  --revision-suffix "cmainapi-${RUN_ID}-${RUN_ATTEMPT}" \
...
probe_json pulse_runtime '/api/pulse-ai/v1/documents/runtime/readiness' '401,403'
probe_json pulse_rag '/api/pulse-ai/v1/rag/readiness' '401,403'
probe_json pulse_doc_pipeline '/api/pulse-ai/v1/documents/pipeline/readiness' '401,403'
probe_sso_state_origin_recovery

WEB_REVISION="$WEB_APP--cmainweb-${RUN_ID}-${RUN_ATTEMPT}"
WEB_STARTED=1
az containerapp update -g "$RESOURCE_GROUP" -n "$WEB_APP" \
  --image "$WEB_IMAGE" \
  --revision-suffix "cmainweb-${RUN_ID}-${RUN_ATTEMPT}" \
  --output none --only-show-errors
...
probe_json proxy_pulse_rag '/api/pulse-ai/v1/rag/readiness' '401,403'
bundle_markers
...
{
  "environment": "test",
  "deploymentType": "cumulative-api-web-after-migrations-051a-052-053",
  ...
  "migration051aVerified": true,
  "migration052Verified": true,
  "migration053Verified": true,
  "pendingApprovalWorkIncluded": true,
  "pulseAiPrivateRuntimeIncluded": true,
  "pulseAiPrivateRagIncluded": true,
  "module006ToyotaHyundaiPipelinesIncluded": true,
  "module006CanonicalRoute": "#toyota-hyundai-pipelines",
  "module006CompatibilityRoutes": ["#psa-modules", "#project-register"],
...
echo 'CURRENT_MAIN_TEST_DEPLOYMENT=COMPLETE'
