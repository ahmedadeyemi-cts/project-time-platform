...
MIGRATION_IMAGE="${CELAR_ANALYTICS_MIGRATION_IMAGE:-}"
DATABASE_URL="${PROJECTPULSE_TEST_DATABASE_URL:-}"
JOB_NAME="${CELAR_ANALYTICS_MIGRATION_JOB_NAME:-}"
LOG_DIR="${CELAR_ANALYTICS_MIGRATION_LOG_DIR:-}"
...
[[ -n "$MIGRATION_IMAGE" ]] || fail "CELAR_ANALYTICS_MIGRATION_IMAGE is not configured."
...
[[ "$JOB_NAME" =~ ^[a-z][a-z0-9-]{0,30}[a-z0-9]$ ]] ||
  fail "CELAR_ANALYTICS_MIGRATION_JOB_NAME is invalid."
...
cleanup() {
  ...
  if (( JOB_CREATED == 1 )); then
    ...
    echo "CELAR_ANALYTICS_MIGRATION_JOB_CLEANUP=COMPLETE"
  fi
  exit "$status"
}
...
az containerapp job create \
  ...
  --memory 0.5Gi \
  "${identity_args[@]}" \
  --secrets "celar-analytics-db-url=$DATABASE_URL" \
  --env-vars \
    PROJECTPULSE_TEST_DATABASE_URL=secretref:celar-analytics-db-url \
    PROJECTPULSE_CELAR_ANALYTICS_MIGRATION_MODE="$MODE" \
  --tags \
    projectpulse-scope="migrations-054-055-test" \
...
echo "CELAR_ANALYTICS_MIGRATION_JOB_CREATE=COMPLETE mode=$MODE"
...
echo "CELAR_ANALYTICS_MIGRATION_JOB_EXECUTION=$EXECUTION_NAME"
...
for attempt in $(seq 1 180); do
  ...
  case "$STATUS" in
    Succeeded)
      emit_logs
      echo "CELAR_ANALYTICS_MIGRATION_JOB_STATUS=SUCCEEDED mode=$MODE"
      exit 0
      ;;
    Failed|Stopped|Canceled|Degraded)
      emit_logs
      echo "CELAR_ANALYTICS_MIGRATION_JOB_STATUS=$STATUS mode=$MODE" >&2
      break
      ;;
  esac
  sleep 5
...
fail "The private-network Celar AI/Analytics migration job did not succeed in $MODE mode."
