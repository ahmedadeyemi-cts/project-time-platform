...
EXPECTED_RELEASE_COMMIT="436eaa60c88442eae1365233569f0bc6f4271a9b"
...
echo "MODULE_012_037_PERMISSION_MIGRATION_JOB_REGISTRY_AUTH=EPHEMERAL_AZURE_TOKEN"
...
cleanup() {
  ...
  if (( JOB_CREATED == 1 )); then
    echo "MODULE_012_037_PERMISSION_MIGRATION_JOB_CLEANUP=STARTED"
    ...
    echo "MODULE_012_037_PERMISSION_MIGRATION_JOB_CLEANUP=COMPLETE"
  fi
  exit "$status"
}
trap cleanup EXIT INT TERM

echo "MODULE_012_037_PERMISSION_MIGRATION_JOB_CREATE=STARTED"
az containerapp job create \
  ...
  --tags \
    projectpulse-scope=module-012-037-permissions-test \
    projectpulse-release="$EXPECTED_RELEASE_COMMIT" \
  --output none \
  --only-show-errors
JOB_CREATED=1
unset REGISTRY_PASSWORD DATABASE_URL
echo "MODULE_012_037_PERMISSION_MIGRATION_JOB_CREATE=COMPLETE"
...
echo "MODULE_012_037_PERMISSION_MIGRATION_JOB_EXECUTION=$EXECUTION_NAME"
...
for attempt in $(seq 1 90); do
  ...
  case "$EXECUTION_STATUS" in
    Succeeded)
      echo "MODULE_012_037_PERMISSION_MIGRATION_JOB_STATUS=Succeeded"
      exit 0
      ;;
    Failed|Canceled)
      echo "MODULE_012_037_PERMISSION_MIGRATION_JOB_STATUS=$EXECUTION_STATUS" >&2
      break
      ;;
  esac
  sleep 5
...
fail "The private-network Module 012/037 permission migration verification job did not succeed."
