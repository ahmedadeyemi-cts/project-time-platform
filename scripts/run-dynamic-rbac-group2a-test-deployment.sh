...
EXPECTED_RELEASE_COMMIT="175579682b24c27daa8ca7b73c348e4ff4cab687"
CONTROL_ROOT="${1:-}"
RELEASE_ROOT="${2:-}"
TARGET_COMMIT="${3:-}"
...
API_REVISION=""
WEB_REVISION=""
PROBE_DIR="${RUNNER_TEMP:-/tmp}/dynamic-rbac-group2a-probes"
mkdir -p "$PROBE_DIR"
...
restore_web() {
  ...
  if [[ "$active" == "$WEB_IMAGE" ]]; then
    suffix="drgwebrb-${RUN_ID}-${RUN_ATTEMPT}"
    ...
    echo "DYNAMIC_RBAC_GROUP2A_WEB_ROLLBACK=COMPLETE"
  else
...
restore_api() {
  ...
  if [[ "$active" == "$API_IMAGE" ]]; then
    suffix="drgapirb-${RUN_ID}-${RUN_ATTEMPT}"
    ...
    echo "DYNAMIC_RBAC_GROUP2A_API_ROLLBACK=COMPLETE"
  else
...
probe_api() {
  ...
  for attempt in $(seq 1 18); do
    local separator='?'
    [[ "$url" == *\?* ]] && separator='&'
    local candidate="$url${separator}release_check=drg-${RUN_ID}-${RUN_ATTEMPT}-${attempt}"
    local args=(
      -sS -L --max-time 30
      -X "$method"
      -D "$headers_file"
    ...
    if [[ ",$expected," == *",$status,"* && "$type" == application/json* ]]; then
      ...
      echo "DYNAMIC_RBAC_GROUP2A_PROBE_${name^^}=HTTP_$status"
      return 0
    fi
    sleep 5
  done
...
API_DIGEST="$("$BUILD_SCRIPT" "$ACR_NAME" "$API_REPOSITORY:dynamic-rbac-group2a-$TARGET_COMMIT" "$RELEASE_ROOT/deployment/containers/api/Dockerfile" "$RELEASE_ROOT")"
WEB_DIGEST="$("$BUILD_SCRIPT" "$ACR_NAME" "$WEB_REPOSITORY:dynamic-rbac-group2a-$TARGET_COMMIT" "$RELEASE_ROOT/deployment/containers/web/Dockerfile" "$RELEASE_ROOT")"
...
echo "DYNAMIC_RBAC_GROUP2A_API_IMAGE=$API_IMAGE"
echo "DYNAMIC_RBAC_GROUP2A_WEB_IMAGE=$WEB_IMAGE"

API_SUFFIX="drgapi-${RUN_ID}-${RUN_ATTEMPT}"
...
probe_api rbac_users GET '/api/rbac/v1/users' '401,403'
probe_api rbac_validate POST '/api/rbac/v1/policies/validate' '401,403' '{"reason":"deployment protected boundary probe","changes":[]}'
probe_api rbac_membership POST '/api/rbac/v1/role-memberships/assign' '401,403' '{"userId":"00000000-0000-0000-0000-000000000000","roleCode":"ENGINEERING","reason":"deployment protected boundary probe"}'
probe_api platform_overview GET '/api/platform-operations/overview' '401,403'
probe_api platform_apis GET '/api/platform-operations/apis' '401,403'
probe_api platform_evidence GET '/api/platform-operations/evidence' '401,403'
probe_api platform_architecture GET '/api/platform-operations/architecture' '401,403'
echo "DYNAMIC_RBAC_GROUP2A_PROTECTED_BOUNDARY=PASS"
...
echo "DYNAMIC_RBAC_GROUP2A_API_VALIDATION=PASS"

WEB_SUFFIX="drgweb-${RUN_ID}-${RUN_ATTEMPT}"
...
# response proves the public HTTPS origin reaches endpoint authorization.
...
for attempt in $(seq 1 24); do
  BUSTER="drg-${RUN_ID}-${RUN_ATTEMPT}-${attempt}"
  ...
  if [[ "$HTML_STATUS" == 200 ]]; then
    ...
    if [[ "$JS_STATUS" == 200 ]] \
      && grep -Fq '/api/rbac/v1/bootstrap' "$PROBE_DIR/app.js" \
      && grep -Fq '/api/rbac/v1/matrix' "$PROBE_DIR/app.js" \
      && grep -Fq 'Role-Based Access Control' "$PROBE_DIR/app.js" \
      && grep -Fq 'No 70-module requirement' "$PROBE_DIR/app.js" \
      && grep -Fq 'Permanent organization-wide Full Control' "$PROBE_DIR/app.js" \
      && grep -Fq 'Search by page name' "$PROBE_DIR/app.js" \
      && grep -Fq 'projectpulse-more-intuitive-name' "$PROBE_DIR/app.js" \
      && grep -Fq '/api/platform-operations/overview' "$PROBE_DIR/app.js" \
      && grep -Fq '/api/platform-operations/evidence' "$PROBE_DIR/app.js" \
      && grep -Fq '/api/platform-operations/architecture' "$PROBE_DIR/app.js" \
      && grep -Fq 'Show volume details' "$PROBE_DIR/app.js" \
      && grep -Fq 'Hide volume details' "$PROBE_DIR/app.js" \
      && ! grep -Fq 'Expected 12 roles and 70 modules' "$PROBE_DIR/app.js" \
      && ! grep -Fq 'Role-policy foundation is incomplete' "$PROBE_DIR/app.js" \
      && ! grep -Fq 'The published permission matrix is incomplete' "$PROBE_DIR/app.js"; then
      SERVED_READY=true
      break
    fi
  fi
...
[[ "$SERVED_READY" == true ]] || fail "The served web bundle did not expose the dynamic RBAC, name-only More, and Group 2A release markers."
...
echo "DYNAMIC_RBAC_GROUP2A_WEB_VALIDATION=PASS"

mkdir -p "$CONTROL_ROOT/evidence"
cat > "$CONTROL_ROOT/evidence/dynamic-rbac-group2a-test-deployment.json" <<JSON
{
  "environment": "test",
  ...
  "rbacContract": "projectpulse-rbac-v1",
  "fixedModuleCountRequired": false,
  "superAdministratorInvariant": "permanent-organization-wide-full-control-active-modules",
  "roleMembershipMutationPerformedByDeployment": false,
  "policyPublishPerformedByDeployment": false,
  "moduleCatalogMutationPerformedByDeployment": false,
  "module012": "dynamic-rbac-administration-served",
  "module037": "dynamic-read-only-matrix-served",
  "group2A": "modules-013-016-068-preserved-and-served",
  "moreMenu": "name-only-permission-and-view-as-aware",
...
echo "DYNAMIC_RBAC_GROUP2A_TEST_DEPLOYMENT=COMPLETE"
