...
for file in "$WORKFLOW" "$MIGRATOR" "$CI_WORKFLOW" "$GUIDE"; do
...
grep -Fq 'PROJECTPULSE_TEST_DATABASE_URL' "$WORKFLOW" || fail "Test database secret guard is missing."
...
[[ -n "$migration_line" && -n "$api_deploy_line" ]] || fail "Cannot determine migration/deployment ordering."
(( migration_line < api_deploy_line )) || fail "Migrations must complete before the API deployment starts."
