...
install -m 0444 "$ROOT/database/migrations/106_module025_sow_sell_register.sql" "$CONTEXT/migration.sql"
printf '%s\n' "$RELEASE" > "$CONTEXT/release-commit"
(cd "$CONTEXT" && sha256sum migration.sql release-commit > SHA256SUMS)
...
psql -X -v ON_ERROR_STOP=1 --file migration.sql
...
COPY migration.sql release-commit SHA256SUMS entrypoint.sh ./
RUN chmod 0444 migration.sql release-commit SHA256SUMS && chmod 0555 entrypoint.sh
...
jq -n --arg source "$RELEASE" --arg image "$MAIN_RELEASE_MIGRATION_IMAGE" \
  '{status:"applied_and_verified",migration:"106_module025_sow_sell_register",sourceCommit:$source,image:$image,productionMutation:false}' \
