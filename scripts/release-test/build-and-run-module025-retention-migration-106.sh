...
(cd "$CONTEXT" && sha256sum migration-106.sql migration-110.sql release-commit > SHA256SUMS)
...
COPY migration-106.sql migration-110.sql release-commit SHA256SUMS entrypoint.sh ./
RUN chmod 0444 migration-106.sql migration-110.sql release-commit SHA256SUMS && chmod 0555 entrypoint.sh
...
jq -n --arg source "$RELEASE" --arg image "$MAIN_RELEASE_MIGRATION_IMAGE" \
  '{status:"applied_and_verified",migrations:["106_module025_sow_sell_register","110_module025_ungenerated_draft_delete"],sourceCommit:$source,image:$image,productionMutation:false}' \
