...
MIGRATION_SCOPE="${RELIABILITY_MIGRATION_SCOPE:-deepseek-secret-schema-test}"
...
validate_job_ownership() {
  ...
  jq -e \
    ...
    --arg databaseUser "$DATABASE_USER" '
      ...
      (.tags["projectpulse-migration"] == "096-097-project-planning-document-authority") and
...
jq -n \
  ...
  --arg databaseUser "$DATABASE_USER" '
  {
    ...
    tags: {
      ...
      "projectpulse-migration": "096-097-project-planning-document-authority"
    },
    properties: {
...
for _ in $(seq 1 180); do
  ...
  case "$STATUS" in
    Succeeded)
      echo "DEEPSEEK_SECRET_SCHEMA_JOB=SUCCEEDED"
      az containerapp job logs show -g "$RESOURCE_GROUP" -n "$JOB_NAME" --execution "$EXECUTION_NAME" --container "$JOB_NAME" --tail 250 --only-show-errors
      exit 0
      ;;
    Failed|Stopped|Canceled|Cancelled|Degraded)
      echo "DEEPSEEK_SECRET_SCHEMA_JOB=$STATUS" >&2
      break
      ;;
  esac
  sleep 5
