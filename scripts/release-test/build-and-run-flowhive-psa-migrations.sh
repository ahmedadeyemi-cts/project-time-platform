...
legacy=['103_module_066_flowhive_enterprise_psa_revamp.sql','104_flowhive_bounded_ai_execution.sql','105_flowhive_reviewed_regeneration.sql','106_module025_sow_sell_register.sql']
successor=legacy+['107_module_066_operation_authorization_and_raid_actor.sql']
selected=[x['file'] for x in approval['migrations']]
if selected not in (legacy, successor): raise SystemExit('Unexpected migration set')
checks=[]
...
MIGRATION_SUFFIX="$(jq -r '.migrations[-1].file' "$CONTROL_ROOT/.github/flowhive-psa-protected-test-candidate.json" | sed -E 's/^([0-9]+)_.*/\1/')"
export MAIN_RELEASE_MIGRATION_SCOPE="flowhive-enterprise-psa-103-${MIGRATION_SUFFIX}-test"
...
MIGRATIONS_JSON="$(jq -c '[.migrations[].file | sub("\\.sql$"; "")]' "$CONTROL_ROOT/.github/flowhive-psa-protected-test-candidate.json")"
