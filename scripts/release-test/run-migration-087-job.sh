...
MIGRATION_IMAGE="${M087_MIGRATION_IMAGE:-}"
JOB_NAME="${M087_MIGRATION_JOB_NAME:-}"
...
CONTROL_SHA="${M087_CONTROL_SHA:-}"
RELEASE_COMMIT="${M087_RELEASE_COMMIT:-}"
MIGRATION_SCOPE="${M087_MIGRATION_SCOPE:-pr670-m087-protected-test}"
RUN_SCOPE="${GITHUB_RUN_ID:-unknown}-${GITHUB_RUN_ATTEMPT:-unknown}"
LOGIN_PASSWORD="${PROJECTPULSE_M087_PASSWORD:-}"
...
[[ "$MIGRATION_IMAGE" == "$ACR_NAME.azurecr.io/"*@sha256:* ]] \
  || fail "M087_MIGRATION_IMAGE must be an immutable digest in the approved Test ACR."
[[ "$JOB_NAME" =~ ^[a-z][a-z0-9-]{0,30}[a-z0-9]$ ]] \
  || fail "M087_MIGRATION_JOB_NAME is invalid."
[[ "$CONTROL_SHA" =~ ^[0-9a-f]{40}$ ]] || fail "M087_CONTROL_SHA must be an exact commit."
[[ "$RELEASE_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail "M087_RELEASE_COMMIT must be an exact commit."
[[ "$MIGRATION_SCOPE" =~ ^[a-z0-9][a-z0-9-]{2,62}$ ]] || fail "M087_MIGRATION_SCOPE is invalid."
...
[[ ${#LOGIN_PASSWORD} -ge 12 ]] \
  || fail "PROJECTPULSE_M087_PASSWORD must be supplied through the protected Test environment."
...
[[ "${UAMI_SUBSCRIPTION,,}" == "${SUBSCRIPTION_ID,,}" ]] \
  || fail "The migrator UAMI is outside the logged-in Test subscription."

ACTUAL_IDENTITY="$(az identity show --ids "$MIGRATOR_IDENTITY" --query id -o tsv --only-show-errors)"
[[ "${ACTUAL_IDENTITY,,}" == "${MIGRATOR_IDENTITY,,}" ]] \
  || fail "The protected Test migrator UAMI could not be resolved exactly."
...
' <<<"$API_JSON" >/dev/null \
  || fail "The protected Test migrator UAMI is not assigned to the API app."
...
arm_get_job() {
  ...
  token="$(az account get-access-token --resource-type arm --query accessToken -o tsv --only-show-errors)" \
    || return 1
  [[ -n "$token" ]] || return 1
...
validate_job_ownership() {
  ...
  jq -e \
    ...
    --arg databaseUser "$DATABASE_USER" '
      ...
      (.tags["projectpulse-migration"] == "087") and
      ...
      (([.properties.configuration.secrets[].name] | sort) ==
        ["m087-login-password", "m087-main-db-password"]) and
      (.properties.configuration.secrets[]
        | select(.name == "m087-main-db-password")
        | .keyVaultUrl == $databaseSecretUri) and
      ...
      (
        ...
        {name: "PGPASSWORD", value: null, secretRef: "m087-main-db-password"},
        ...
        {name: "PROJECTPULSE_M087_PASSWORD", value: null, secretRef: "m087-login-password"},
...
cleanup() {
  ...
  trap - EXIT INT TERM
  set +e
  unset PROJECTPULSE_M087_PASSWORD LOGIN_PASSWORD
    ...
    if [[ "$http_status" == "200" ]]; then
      if ! validate_job_ownership "$JOB_RESPONSE"; then
        ...
        if (( START_ATTEMPTED == 1 )); then
          stop_nonterminal_executions || exit_status=1
        fi
      ...
      echo "ERROR: Could not prove temporary Migration 087 job cleanup." >&2
      exit_status=1
    fi
  fi
...
PREFLIGHT_STATUS="$(arm_get_job "$JOB_RESPONSE")" \
  || fail "Could not perform the Migration 087 job ARM preflight."
[[ "$PREFLIGHT_STATUS" == "404" ]] \
  || fail "A Container Apps Job already exists with the guarded Migration 087 job name."
  ...
  --arg databaseUser "$DATABASE_USER" '
  {
    ...
    properties: {
      ...
      configuration: {
        ...
        secrets: [
          {
            name: "m087-main-db-password",
            keyVaultUrl: $databaseSecretUri,
            identity: $identity
          },
          {
            name: "m087-login-password",
            value: $loginPassword
          }
        ]
      },
      template: {
        containers: [{
          ...
          env: [
            ...
            {name: "PGPASSWORD", secretRef: "m087-main-db-password"},
            {name: "PGPORT", value: $databasePort},
            {name: "PGSSLROOTCERT", value: "system"},
            {name: "PGSSLMODE", value: "verify-full"},
            {name: "PGUSER", value: $databaseUser},
            {name: "PROJECTPULSE_M087_PASSWORD", secretRef: "m087-login-password"},
...
for _ in $(seq 1 60); do
  JOB_STATUS="$(arm_get_job "$JOB_RESPONSE")" \
    || fail "Could not read the temporary Migration 087 job after creation."
  [[ "$JOB_STATUS" == "200" ]] \
    || fail "The temporary Migration 087 job was not readable after creation."
  validate_job_ownership "$JOB_RESPONSE" \
    || fail "The temporary Migration 087 job ownership or immutable configuration did not match."
  ...
  case "$PROVISIONING_STATE" in
    ...
    Failed|Canceled|Cancelled)
      fail "The temporary Migration 087 job provisioning failed."
      ;;
  esac
  sleep 2
done
(( PROVISIONED == 1 )) || fail "The temporary Migration 087 job did not provision successfully."
...
normalize "$EXECUTION_NAME" >/dev/null \
  || fail "Azure did not return the Migration 087 execution name."
...
for _ in $(seq 1 180); do
  ...
  case "$STATUS" in
    Succeeded)
      echo "MIGRATION_087_PRIVATE_NETWORK_JOB=SUCCEEDED"
      exit 0
      ;;
    Failed|Stopped|Canceled|Cancelled|Degraded)
      echo "MIGRATION_087_PRIVATE_NETWORK_JOB=$STATUS" >&2
      break
      ;;
  esac
  sleep 5
...
fail "The protected Test Migration 087 job did not succeed."
