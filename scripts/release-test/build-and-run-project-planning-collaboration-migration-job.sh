...
SELECT
  ...
  EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='project_planning_collaborators' AND column_name='module_code')::text || '|' ||
  EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='project_planning_collaborators' AND column_name='collaboration_level')::text || '|' ||
...
DOCKERFILE_CONTENT
[[ -f "$DOCKERFILE" ]] || fail "Migration 095 Dockerfile was not created in its ACR build context."
...
if [[ -n "$EVIDENCE_ROOT" ]]; then
  ...
  jq -n \
    ...
    '{status:"applied_and_verified",migration:"095_project_planning_collaboration_access",releaseCommit:$releaseCommit,image:$image,environment:"protected-test",privateNetworkJob:true,productionMutation:false}' \
