...
MIGRATION_IMAGE="${MODULE001_MIGRATION_IMAGE:-}"
DATABASE_URL="${PROJECTPULSE_TEST_DATABASE_URL:-}"
JOB_NAME="${MODULE001_MIGRATION_JOB_NAME:-}"
...
[[ -n "$MIGRATION_IMAGE" ]] || fail "MODULE001_MIGRATION_IMAGE is not configured."
[[ -n "$DATABASE_URL" ]] || fail "PROJECTPULSE_TEST_DATABASE_URL is not configured."
[[ "$JOB_NAME" =~ ^[a-z][a-z0-9-]{0,30}[a-z0-9]$ ]] ||
  fail "MODULE001_MIGRATION_JOB_NAME must be a valid Container Apps Job name no longer than 32 characters."
...
echo "MODULE001_MIGRATION_JOB_REGISTRY_AUTH=EPHEMERAL_AZURE_TOKEN"
...
cleanup() {
  ...
  if (( JOB_CREATED == 1 )); then
    echo "MODULE001_MIGRATION_JOB_CLEANUP=STARTED"
    ...
    echo "MODULE001_MIGRATION_JOB_CLEANUP=COMPLETE"
  fi
  exit "$status"
}
trap cleanup EXIT INT TERM

echo "MODULE001_MIGRATION_JOB_CREATE=STARTED"
az containerapp job create \
  ...
  --secrets "module001-db-url=$DATABASE_URL" \
  --env-vars PROJECTPULSE_TEST_DATABASE_URL=secretref:module001-db-url \
  --tags \
    projectpulse-scope=module001-test-migration \
    projectpulse-release="$EXPECTED_RELEASE_COMMIT" \
  --output none \
  --only-show-errors
JOB_CREATED=1
unset REGISTRY_PASSWORD DATABASE_URL
echo "MODULE001_MIGRATION_JOB_CREATE=COMPLETE"
...
echo "MODULE001_MIGRATION_JOB_EXECUTION=$EXECUTION_NAME"
...
for attempt in $(seq 1 90); do
  ...
  case "$EXECUTION_STATUS" in
    Succeeded)
      echo "MODULE001_MIGRATION_JOB_STATUS=Succeeded"
      exit 0
      ;;
    Failed|Canceled)
      echo "MODULE001_MIGRATION_JOB_STATUS=$EXECUTION_STATUS" >&2
      break
      ;;
  esac
  sleep 5
...
fail "The private-network Module 001 migration job did not succeed."
