...
MIGRATION_IMAGE="${INTUITIVE_PERMISSIONS_MIGRATION_IMAGE:-}"
DATABASE_URL="${PROJECTPULSE_TEST_DATABASE_URL:-}"
JOB_NAME="${INTUITIVE_PERMISSIONS_MIGRATION_JOB_NAME:-}"

fail() { echo "ERROR: $*" >&2; exit 1; }
normalize_az_value() {
  case "${1:-}" in ""|None|null) return 1 ;; *) printf '%s\n' "$1" ;; esac
}
mask_value() {
  local value="$1"
  value="${value//%/%25}"
...
[[ -n "$MIGRATION_IMAGE" ]] || fail "INTUITIVE_PERMISSIONS_MIGRATION_IMAGE is not configured."
[[ -n "$DATABASE_URL" ]] || fail "PROJECTPULSE_TEST_DATABASE_URL is not configured."
[[ "$JOB_NAME" =~ ^[a-z][a-z0-9-]{0,30}[a-z0-9]$ ]] ||
  fail "INTUITIVE_PERMISSIONS_MIGRATION_JOB_NAME must be a valid name no longer than 32 characters."
...
if az containerapp job show --resource-group "$RESOURCE_GROUP" --name "$JOB_NAME" \
...
REGISTRY_PASSWORD="$(az acr login --name "$ACR_NAME" --expose-token \
...
echo "INTUITIVE_PERMISSIONS_MIGRATION_REGISTRY_AUTH=EPHEMERAL_AZURE_TOKEN"
...
cleanup() {
  ...
  if (( JOB_CREATED == 1 )); then
    echo "INTUITIVE_PERMISSIONS_MIGRATION_JOB_CLEANUP=STARTED"
    az containerapp job delete --resource-group "$RESOURCE_GROUP" --name "$JOB_NAME" \
      --yes --output none --only-show-errors || true
    echo "INTUITIVE_PERMISSIONS_MIGRATION_JOB_CLEANUP=COMPLETE"
  fi
  exit "$status"
}
trap cleanup EXIT INT TERM

echo "INTUITIVE_PERMISSIONS_MIGRATION_JOB_CREATE=STARTED"
az containerapp job create \
  ...
  --tags \
    projectpulse-scope=modules-001-012-037-intuitive-permissions-test \
    projectpulse-release="$EXPECTED_RELEASE_COMMIT" \
  --output none \
  --only-show-errors
JOB_CREATED=1
unset REGISTRY_PASSWORD DATABASE_URL
echo "INTUITIVE_PERMISSIONS_MIGRATION_JOB_CREATE=COMPLETE"

EXECUTION_NAME="$(az containerapp job start --resource-group "$RESOURCE_GROUP" \
  --name "$JOB_NAME" --query name --output tsv --only-show-errors)"
normalize_az_value "$EXECUTION_NAME" >/dev/null ||
  fail "Azure did not return the migration job execution name."
echo "INTUITIVE_PERMISSIONS_MIGRATION_JOB_EXECUTION=$EXECUTION_NAME"
...
for attempt in $(seq 1 120); do
  ...
  case "$EXECUTION_STATUS" in
    Succeeded)
      echo "INTUITIVE_PERMISSIONS_MIGRATION_JOB_STATUS=Succeeded"
      exit 0
      ;;
    Failed|Canceled)
      echo "INTUITIVE_PERMISSIONS_MIGRATION_JOB_STATUS=$EXECUTION_STATUS" >&2
      break
      ;;
  esac
  sleep 5
...
az containerapp job logs show --resource-group "$RESOURCE_GROUP" --name "$JOB_NAME" \
  --execution "$EXECUTION_NAME" --container "$JOB_NAME" --tail 250 --only-show-errors >&2 || true
fail "The private-network combined migration job did not succeed."
