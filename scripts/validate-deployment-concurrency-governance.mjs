...
/* Trusted single-file validator. Bundles yaml@2.8.1 (npm integrity sha512-lcYcMxX2PO9XMGvAJkJ3OsNMw+/7FKes7/hgerGUYWIoWu5j/+YQqcZr5JnPZWzOsEBgMbSbiSTn/dv/69Mkpw==) to avoid executing PR-controlled or network-installed parsers.
Copyright Eemeli Aro <eemeli@gmail.com>
Permission to use, copy, modify, and/or distribute this software for any purpose with or without fee is hereby granted, provided that this copyright and permission notice appear in all copies.
THE SOFTWARE IS PROVIDED "AS IS" AND THE AUTHOR DISCLAIMS ALL WARRANTIES, INCLUDING IMPLIED WARRANTIES OF MERCHANTABILITY AND FITNESS, AND ALL LIABILITY FOR SPECIAL, DIRECT, INDIRECT, OR CONSEQUENTIAL DAMAGES OR LOSS OF USE, DATA, OR PROFITS. */
  ...
  "../yaml-governance-bundle/node_modules/yaml/dist/compose/resolve-flow-collection.js"(exports) {
    ...
    function resolveFlowCollection({ composeNode, composeEmptyNode }, ctx, fc, onError, tag) {
      ...
      if (ce && ce.source === expectedEnd)
        ...
        const msg = atRoot2 ? `${name} must end with a ${expectedEnd}` : `${name} in block collection must be sufficiently indented and end with a ${expectedEnd}`;
        onError(offset, atRoot2 ? "MISSING_CHAR" : "BAD_INDENT", msg);
...
var deploymentManifest = /* @__PURE__ */ new Map([
  ...
  [".github/workflows/projectpulse-deploy-combined-modules-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-dashboard-calendar-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-dashboard-hotfix-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-direct-runtime-verified-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-dynamic-rbac-group2a-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-global-session-invalidation-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-group2b-group3-module064-foundation-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-group4-notifications-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-intuitive-permissions-ptc-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-microsoft-integration-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-microsoft-preview-mail-runtime-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-module-001-blank-page-hotfix-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-module-001-simplified-timer-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-module-001-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-module-001-timer-hotfix-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-module-001-view-sync-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-module-005-project-expense-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-module-008-audit-recovery-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-module-012-037-permissions-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-module001-ptc-timer-dom-module026-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-modules-card-numbers-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-modules-page-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-open-pr-reconciliation-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-pr55-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-production.yml", "production"],
  [".github/workflows/projectpulse-deploy-runtime-data-hotfix-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-runtime-direct-timer-recovery-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-scoped-rbac-catalog-hotfix-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-scoped-rbac-test.yml", "test"],
  [".github/workflows/projectpulse-deploy-security-admin-repair-test.yml", "test"],
  ...
  [".github/workflows/projectpulse-run-group4-migration-050-test.yml", "test"]
]);
var scopeWorkflowPaths = /* @__PURE__ */ new Set([
...
var contractReplacements = /* @__PURE__ */ new Map([
  ["scripts/validate-admin-experience-008-009-test-deployment.sh", `require "$WORKFLOW" 'projectpulse-deploy-admin-experience-008-009-test'`],
  ["scripts/validate-dynamic-rbac-group2a-test-deployment.sh", `require "$DEPLOY" 'group: projectpulse-deploy-dynamic-rbac-group2a-test'`],
  ["scripts/validate-global-session-invalidation-test-deployment.sh", `require "$DEPLOY" 'group: projectpulse-deploy-global-session-invalidation-test'`],
  ["scripts/validate-module001-ptc-timer-dom-module026-test-deployment.sh", `require "$DEPLOY" 'group: projectpulse-deploy-module001-module026-microsoft-runtime-test'`],
  ["scripts/validate-open-pr-reconciliation-test-deployment.sh", `require "$DEPLOY" 'group: projectpulse-deploy-open-pr-reconciliation-test'`],
  ["scripts/validate-security-admin-repair-test-deployment.sh", `require "$DEPLOY" 'group: projectpulse-deploy-security-admin-repair-test'`],
  ["scripts/validate-superadmin-sso-expense-module006-test-deployment.sh", `require "$DEPLOY" 'group: projectpulse-deploy-superadmin-sso-expense-module006-test'`],
  ["scripts/validate-view-as-drawer-test-deployment.sh", `require "$DEPLOY" 'group: projectpulse-deploy-view-as-drawer-test'`]
]);
var contractScriptPaths = new Set(contractReplacements.keys());
var rootScopeBlock = `          # DEPLOYMENT_CONCURRENCY_GOVERNANCE_SCOPE_START
          ...
          git show "$GOVERNANCE_BASE_SHA:${VALIDATOR_PATH}" > "\${RUNNER_TEMP}/${TRUSTED_TEMP_NAME}"
          set +e
          node "\${RUNNER_TEMP}/${TRUSTED_TEMP_NAME}" --repo-root "\${GITHUB_WORKSPACE}" --base-ref "$GOVERNANCE_BASE_SHA" --classify-bootstrap-scope
          governance_status=$?
          set -e
...
`;
var controlScopeBlock = `          # DEPLOYMENT_CONCURRENCY_GOVERNANCE_SCOPE_START
          GOVERNANCE_BASE_SHA='\${{ github.event.pull_request.base.sha }}'
          git -C control fetch origin "$GOVERNANCE_BASE_SHA" --no-tags
          git -C control cat-file -e "$GOVERNANCE_BASE_SHA^{commit}"
          git -C control show "$GOVERNANCE_BASE_SHA:${VALIDATOR_PATH}" > "\${RUNNER_TEMP}/${TRUSTED_TEMP_NAME}"
          set +e
          node "\${RUNNER_TEMP}/${TRUSTED_TEMP_NAME}" --repo-root "\${GITHUB_WORKSPACE}/control" --base-ref "$GOVERNANCE_BASE_SHA" --classify-bootstrap-scope
          governance_status=$?
          set -e
          if [[ $governance_status -eq 0 ]]; then
            echo 'DEPLOYMENT_CONCURRENCY_GOVERNANCE_SCOPE=PASS'
...
var defaultRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");
var repoRoot = resolve(argument("--repo-root") ?? defaultRoot);
var atRoot = (path) => join(repoRoot, path);
var failures = [];
var fail = (message) => failures.push(message);
function git(args) {
  return execFileSync("git", args, { cwd: repoRoot, encoding: "utf8" });
}
function exactBaseRef() {
...
function treeEntry(ref, path) {
  const output = git(["ls-tree", "-z", ref, "--", path]);
  if (output.length === 0) return null;
  const match = output.match(/^(\d{6}) (\w+) ([0-9a-f]{40})\t([^\0]+)\0$/);
  if (!match || match[4] !== path) throw new Error(`${path}: malformed Git tree entry at ${ref}`);
  return { mode: match[1], type: match[2], sha: match[3], path: match[4] };
}
function requireRegularFile(ref, path) {
  const entry = treeEntry(ref, path);
  if (!entry || entry.mode !== "100644" || entry.type !== "blob") {
    throw new Error(`${path}: expected a regular 100644 Git blob at ${ref}`);
  }
  return entry;
}
function fileAt(ref, path) {
  ...
  if (entry.mode !== "100644" || entry.type !== "blob") {
    throw new Error(`${path}: non-regular Git object is prohibited at ${ref}`);
  }
...
function changedEntries(base) {
  const tokens = git(["diff", "--name-status", "--no-renames", "-z", `${base}...HEAD`]).split("\0");
  if (tokens.at(-1) === "") tokens.pop();
  const entries = [];
  for (let index = 0; index < tokens.length; ) {
    const status = tokens[index++];
    if (/^[RC]/.test(status)) {
      entries.push({ status, path: tokens[index++], destination: tokens[index++] });
    } else {
...
function parseWorkflow(name, source) {
  ...
  let document;
  try {
    document = (0, import_yaml.parseDocument)(source, {
      prettyErrors: false,
      strict: true,
      uniqueKeys: true,
      version: "1.2"
    ...
    return { root: {}, values: () => [], jobs: [], issues };
  }
  for (const error of document.errors) issues.push(`${name}: YAML error: ${error.message}`);
  for (const warning of document.warnings) issues.push(`${name}: YAML warning: ${warning.message}`);
  (0, import_yaml.visit)(document, (_key, node) => {
    if ((0, import_yaml.isAlias)(node) || node?.anchor) issues.push(`${name}: YAML anchors and aliases are prohibited`);
  });
  let root = {};
  if (issues.length === 0) {
    try {
      const parsed = document.toJS({ mapAsMap: false, maxAliasCount: 0 });
      if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        issues.push(`${name}: workflow root must be a mapping`);
      } else {
        root = parsed;
      }
    } catch (error) {
      issues.push(`${name}: YAML conversion failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  const values = (path) => {
    ...
    for (const segment of path.split(".")) {
      if (!value || typeof value !== "object" || Array.isArray(value) || !Object.prototype.hasOwnProperty.call(value, segment)) return [];
      value = value[segment];
    }
    const scalar = scalarValue(value);
    return scalar === null ? [] : [scalar];
  };
  const jobsMapping = root.jobs && typeof root.jobs === "object" && !Array.isArray(root.jobs) ? root.jobs : {};
  if (!root.jobs || jobsMapping !== root.jobs) issues.push(`${name}: jobs must be a mapping`);
  const jobs = Object.entries(jobsMapping).map(([jobName, rawJob]) => {
    const job = rawJob && typeof rawJob === "object" && !Array.isArray(rawJob) ? rawJob : null;
    if (!job) issues.push(`${name}: job ${jobName} must be a mapping`);
    const steps = Array.isArray(job?.steps) ? job.steps : [];
    return {
      name: jobName,
      environments: typeof job?.environment === "string" ? [job.environment] : [],
      jobUses: typeof job?.uses === "string" ? [job.uses] : [],
      stepUses: steps.flatMap((step) => step && typeof step === "object" && !Array.isArray(step) && typeof step.uses === "string" ? [step.uses] : []),
      condition: typeof job?.if === "string" ? job.if : ""
    };
  });
  return { root, values, jobs, issues };
}
function expectedGroup(environmentClass) {
  if (environmentClass === "test") return "projectpulse-deploy-test";
  if (environmentClass === "production") return "projectpulse-deploy-production";
  return "projectpulse-deploy-${{ inputs.environment }}";
}
function validateManifestWorkflow(path, environmentClass, source) {
  const parsed = parseWorkflow(path, source);
  parsed.issues.forEach(fail);
  if (parsed.jobs.length !== 1) fail(`${path}: governed controller must contain exactly one protected job`);
  const job = parsed.jobs[0];
  if (!job) return;
  const expectedEnvironment = environmentClass === "dynamic" ? "${{ inputs.environment }}" : environmentClass;
  if (job.environments.length !== 1 || job.environments[0] !== expectedEnvironment) {
    fail(`${path}: job environment must be exactly ${expectedEnvironment}`);
  }
  if (job.jobUses.length > 0) fail(`${path}: governed deployment cannot delegate through a reusable job`);
  const group = parsed.values("concurrency.group");
  const queue = parsed.values("concurrency.queue");
  const cancel = parsed.values("concurrency.cancel-in-progress");
  if (group.length !== 1 || group[0] !== expectedGroup(environmentClass)) fail(`${path}: incorrect environment-wide concurrency group`);
  if (queue.length !== 1 || queue[0] !== "max") fail(`${path}: queue: max is required`);
  if (cancel.length !== 1 || cancel[0] !== "false") fail(`${path}: cancel-in-progress: false is required`);
  if (environmentClass === "dynamic") {
    const triggers = parsed.root.on;
    if (!triggers || typeof triggers !== "object" || Array.isArray(triggers) || JSON.stringify(Object.keys(triggers)) !== JSON.stringify(["workflow_dispatch"])) {
      fail(`${path}: dynamic rollback must have only the workflow_dispatch trigger`);
    }
    const environmentInput = triggers?.workflow_dispatch?.inputs?.environment;
    if (!environmentInput || typeof environmentInput !== "object" || Array.isArray(environmentInput) || environmentInput.required !== true || environmentInput.default !== "test" || environmentInput.type !== "choice" || JSON.stringify(environmentInput.options) !== JSON.stringify(["test", "production"])) {
      fail(`${path}: dynamic environment input must be the exact required Test/Production choice`);
    }
    const expectedCondition = "github.ref == 'refs/heads/main' && (inputs.environment == 'test' || inputs.environment == 'production')";
    if (job.condition !== expectedCondition) {
      fail(`${path}: dynamic rollback job must enforce the exact Test/Production runtime allowlist`);
    }
  }
}
function verifyWorktreeStructure() {
  for (const [path, environmentClass] of deploymentManifest) {
    if (!existsSync(atRoot(path))) {
      fail(`${path}: immutable deployment manifest path is missing`);
      continue;
    }
    validateManifestWorkflow(path, environmentClass, current(path));
  }
  ...
  for (const name of readdirSync(workflowDirectory).filter((item) => /\.ya?ml$/i.test(item)).sort()) {
    ...
    if (deploymentManifest.has(path)) continue;
    const source = current(path);
    const parsed = parseWorkflow(path, source);
    parsed.issues.forEach(fail);
    const protectedEvidence = parsed.jobs.some((job) => job.environments.some((value) => ["test", "production", "${{ inputs.environment }}"].includes(value)) || job.jobUses.length > 0 && /deploy|rollback|migration/i.test(job.name) || job.stepUses.some((value) => value.startsWith("azure/login@")));
    const credentialEvidence = /id-token:\s*write|\$\{\{\s*(?:vars|secrets)\.AZURE_|\baz\s+(?:containerapp|acr|account|login)\b/i.test(source);
    if (protectedEvidence || credentialEvidence) {
      fail(`${path}: environment-mutating workflow is missing from the immutable deployment manifest`);
    }
  }
}
function verifyRepository() {
  requireRegularFile("HEAD", SENTINEL_PATH);
  if (current(SENTINEL_PATH) !== SENTINEL_CONTENT) fail(`${SENTINEL_PATH}: exact governance sentinel is required`);
  requireRegularFile("HEAD", VALIDATOR_PATH);
  requireRegularFile("HEAD", WORKFLOW_PATH);
  for (const [path] of deploymentManifest) requireRegularFile("HEAD", path);
  for (const name of readdirSync(atRoot(".github/workflows")).filter((item) => /\.ya?ml$/i.test(item))) {
    requireRegularFile("HEAD", `.github/workflows/${name}`);
  }
...
function normalizeDeploymentControl(path, source) {
  let normalized = normalizeConcurrency(source);
  if (path === ".github/workflows/projectpulse-rollback.yml") {
    normalized = normalized.replace("    if: github.ref == 'refs/heads/main' && (inputs.environment == 'test' || inputs.environment == 'production')", "    if: __PROTECTED_DYNAMIC_ENVIRONMENT_GUARD__").replace("    if: github.ref == 'refs/heads/main'", "    if: __PROTECTED_DYNAMIC_ENVIRONMENT_GUARD__");
  }
  return normalized;
}
function removeExactScopeBlock(source, expected, path) {
  const starts = source.split("DEPLOYMENT_CONCURRENCY_GOVERNANCE_SCOPE_START").length - 1;
  const ends = source.split("DEPLOYMENT_CONCURRENCY_GOVERNANCE_SCOPE_END").length - 1;
  if (starts !== 1 || ends !== 1 || !source.includes(expected)) {
    fail(`${path}: expected exactly one immutable base-SHA validator scope block`);
    return source;
  }
...
function verifySingleContractReplacement(path, before, after) {
  ...
  const changed = beforeLines.map((line, index2) => line === afterLines[index2] ? -1 : index2).filter((index2) => index2 >= 0);
  if (changed.length !== 1) {
    fail(`${path}: expected exactly one deployment concurrency literal change`);
    return;
  }
  const index = changed[0];
  const expectedOld = contractReplacements.get(path);
  if (!expectedOld || !beforeLines[index].includes(expectedOld)) {
    fail(`${path}: the exact legacy concurrency assertion was not replaced`);
    return;
  }
  const expectedNew = expectedOld.replace(/projectpulse-deploy-[A-Za-z0-9_-]+(?='?$)/, "projectpulse-deploy-test");
  const replaced = beforeLines[index].replace(expectedOld, expectedNew);
  if (afterLines[index] !== replaced) {
    fail(`${path}: only the exact legacy lock assertion may become projectpulse-deploy-test`);
  }
}
function verifyBootstrapPackage(base) {
  ...
  const sentinel = changed.get(SENTINEL_PATH);
  if (sentinel?.status === "A") requireRegularFile("HEAD", SENTINEL_PATH);
  if (!sentinel || sentinel.status !== "A" || current(SENTINEL_PATH) !== SENTINEL_CONTENT) {
    fail(`${SENTINEL_PATH}: exact regular-file sentinel addition is required`);
  }
  for (const path of [VALIDATOR_PATH, WORKFLOW_PATH]) {
    if (changed.has(path) || current(path) !== fileAt(base, path)) fail(`${path}: trusted controls must remain byte-identical to base`);
  }
  for (const path of scopeWorkflowPaths) if (!changed.has(path)) fail(`${path}: immutable compatibility block is required`);
  for (const path of contractScriptPaths) if (!changed.has(path)) fail(`${path}: shared-lock contract update is required`);
  for (const entry of entries) {
    const { path, status } = entry;
    if (entry.destination || !["A", "M"].includes(status)) {
      fail(`${path}: rename, copy, deletion, and type changes are prohibited`);
      continue;
    }
    ...
    if (deploymentManifest.has(path)) {
      if (normalizeDeploymentControl(path, after) !== normalizeDeploymentControl(path, before)) {
        fail(`${path}: changed outside top-level concurrency group/queue or the exact rollback environment guard`);
      }
      continue;
    }
    if (scopeWorkflowPaths.has(path)) {
      ...
      if (removeExactScopeBlock(after, expected, path) !== before) fail(`${path}: changed outside immutable governance scope block`);
      continue;
    }
    ...
    fail(`${path}: outside one-time deployment governance package`);
  }
  verifyRepository();
}
function verifyWorkflowRootOfTrust() {
  ...
  if (/^\s+pull_request:\s*$/m.test(source) || /^\s+paths:\s*$/m.test(source)) {
    fail(`${WORKFLOW_PATH}: required governance must use unfiltered pull_request_target`);
  }
  if (/\$\{\{\s*secrets\.|node\s+scripts\/validate-deployment-concurrency-governance\.mjs[\s\S]*--verify-pr/.test(source)) {
...
`;
  const parsed = parseWorkflow("good.yml", good);
  if (parsed.issues.length || parsed.jobs.length !== 1 || parsed.jobs[0].environments[0] !== "test") fail("self-test: valid structural workflow rejected");
  const spoof = good.replace("    environment: test\n", "    env:\n      environment: test\n");
  if (parseWorkflow("spoof.yml", spoof).jobs[0].environments.length !== 0) fail("self-test: env.environment spoof accepted");
  for (const indicator of ["|", "|2", ">2-", "|-2"]) {
    const blockSpoof = good.replace("    environment: test\n", `    notes: ${indicator}
      environment: test
      uses: azure/login@fake
`);
    const blockParsed = parseWorkflow(`block-${indicator}.yml`, blockSpoof);
    if (blockParsed.issues.length || blockParsed.jobs.length !== 1 || blockParsed.jobs[0].environments.length !== 0 || blockParsed.jobs[0].stepUses.includes("azure/login@fake")) {
      fail(`self-test: block scalar spoof ${indicator} accepted`);
    }
  }
  const quotedJob = good.replace("concurrency:\n", '  "ship-unprotected":\n    runs-on: ubuntu-latest\n    steps:\n      - uses: azure/login@fake\nconcurrency:\n');
  if (parseWorkflow("quoted-job.yml", quotedJob).jobs.length !== 2) fail("self-test: quoted job ID was ignored");
  const duplicate = good.replace("    environment: test\n", "    environment: test\n    environment: production\n");
  if (parseWorkflow("duplicate.yml", duplicate).issues.length === 0) fail("self-test: duplicate YAML key was accepted");
  const flow = `name: fixture
on: { workflow_dispatch: {} }
jobs: { deploy: { environment: test, runs-on: ubuntu-latest, steps: [{ uses: azure/login@pinned }] } }
concurrency: { group: projectpulse-deploy-test, queue: max, cancel-in-progress: false }
`;
  const flowParsed = parseWorkflow("flow.yml", flow);
...
function classifyBootstrap(base) {
  const changed = new Set(changedEntries(base).map((entry) => entry.path));
...
function verifyPullRequest(base) {
  if (fileAt(base, VALIDATOR_PATH) === null) {
    fail("trusted validator is absent from the pull-request base");
    return;
  }
  requireRegularFile("HEAD", VALIDATOR_PATH);
  requireRegularFile("HEAD", WORKFLOW_PATH);
  if (current(VALIDATOR_PATH) !== fileAt(base, VALIDATOR_PATH) || current(WORKFLOW_PATH) !== fileAt(base, WORKFLOW_PATH)) {
    fail("trusted validator and workflow cannot authorize their own modification");
    return;
  }
  const result = classifyBootstrap(base);
  if (result === 0) return;
  if (result === 1) {
    fail("invalid governance bootstrap or trusted-control mutation");
    return;
  }
  if (fileAt(base, SENTINEL_PATH) === null) {
    const transitionChanges = changedEntries(base).filter((entry) => entry.path.startsWith(".github/workflows/") || entry.destination?.startsWith(".github/workflows/") || contractScriptPaths.has(entry.path));
    if (transitionChanges.length > 0) {
      fail(`deployment-governance transition blocks these control changes until the sentinel package lands: ${transitionChanges.map((entry) => entry.path).join(", ")}`);
      return;
    }
    selfTest();
    return;
...
try {
  ...
  else throw new Error("Select one explicit governance mode");
} catch (error) {
