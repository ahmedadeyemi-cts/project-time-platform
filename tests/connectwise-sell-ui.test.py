...
with tempfile.TemporaryDirectory(prefix='.connectwise-ui-', dir=web) as folder:
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            assert page.locator('.crm-erp-configuration').get_by_role('button', name='OAuth 2.0', exact=False).is_disabled()
            ...
            page.get_by_label('Access Key (ConnectWise SELL site key)', exact=True).fill('tenant_azure')
            ...
            page.wait_for_function("!document.querySelector('input[autocomplete=\"new-password\"]') || document.querySelector('input[autocomplete=\"new-password\"]').value === ''")
