...
CONTAINER="projectpulse-analytics-055-${GITHUB_RUN_ID:-local}-$$"
DB_USER="projectpulse"
DB_NAME="projectpulse"
DB_PASSWORD="projectpulse-test-only"
MIGRATION="/workspace/database/migrations/055_analytics_center.sql"
ROLLBACK="/workspace/database/rollback/055_analytics_center_rollback.sql"
...
expect_sql_failure() {
  ...
  local expected="$2"
  local label="$3"
  local log="/tmp/analytics-055-${label}.log"
...
INSERT INTO app_feature_catalog (
  ...
  'FINANCIAL_REPORT_CENTER','Financial Report Center','030','#reporting',
  'VIEW_FINANCIAL_REPORT_CENTER','Legacy Group 5 report experience',300,TRUE
);
...
assert_eq 1 "$(value "SELECT COUNT(*) FROM schema_migrations WHERE migration_id='055_analytics_center';")" migration_registered_once
assert_eq 3 "$(value "SELECT COUNT(*) FROM pg_tables WHERE schemaname='public' AND tablename IN ('enterprise_report_runs','enterprise_report_saved_views','enterprise_report_exports');")" analytics_tables_created
assert_eq 4 "$(value "SELECT COUNT(*) FROM app_permissions WHERE permission_code IN ('VIEW_ENTERPRISE_REPORTING','RUN_ENTERPRISE_REPORTING','EXPORT_ENTERPRISE_REPORTING','MANAGE_ENTERPRISE_REPORTING');")" analytics_permissions_created
assert_eq 1 "$(value "SELECT COUNT(*) FROM app_feature_catalog WHERE feature_code='ANALYTICS_CENTER' AND feature_name='Analytics Center' AND is_active=TRUE;")" analytics_feature_created
assert_eq 0 "$(value "SELECT COUNT(*) FROM app_feature_catalog WHERE feature_code='FINANCIAL_REPORT_CENTER' AND is_active=TRUE;")" legacy_feature_deactivated
assert_eq 4 "$(value "SELECT COUNT(*) FROM app_roles r JOIN app_role_permissions rp USING(app_role_id) JOIN app_permissions p USING(app_permission_id) WHERE r.role_code='ACCOUNTING';")" accounting_full_permissions
assert_eq 3 "$(value "SELECT COUNT(*) FROM app_roles r JOIN app_role_permissions rp USING(app_role_id) JOIN app_permissions p USING(app_permission_id) WHERE r.role_code='ENGINEER';")" engineer_scoped_permissions
assert_eq 3 "$(value "SELECT COUNT(*) FROM app_roles r JOIN app_role_permissions rp USING(app_role_id) JOIN app_permissions p USING(app_permission_id) WHERE r.role_code='PROJECT_MANAGER';")" pm_scoped_permissions

psql_exec <<'SQL'
INSERT INTO enterprise_report_runs (
  enterprise_report_run_id, report_code, report_name,
  actual_user_id, effective_user_id, result_status, row_count, result_json
) VALUES (
...
expect_sql_failure "UPDATE enterprise_report_runs SET row_count=2;" 'Analytics Center run and export evidence is immutable.' immutable_run_update
expect_sql_failure "DELETE FROM enterprise_report_exports;" 'Analytics Center run and export evidence is immutable.' immutable_export_delete
...
assert_eq 0 "$(value "SELECT COUNT(*) FROM schema_migrations WHERE migration_id='055_analytics_center';")" rollback_removed_migration
...
assert_eq 1 "$(value "SELECT COUNT(*) FROM app_feature_catalog WHERE feature_code='FINANCIAL_REPORT_CENTER' AND is_active=TRUE;")" rollback_reactivated_legacy_feature

psql_exec -f "$MIGRATION" >/dev/null
assert_eq 1 "$(value "SELECT COUNT(*) FROM schema_migrations WHERE migration_id='055_analytics_center';")" safe_reapply

echo 'ANALYTICS_CENTER_MIGRATION_055=PASS'
