    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            state={'projectId':'project-a','enabled':False,'rowVersion':'revision-1','status':'disabled','message':'Automatic planning is off.',
                   'canManage':True,'myDefault':{'enabled':False,'rowVersion':'pm-default-1','canManage':True},
                   'defaults':{'enabled':False,'rowVersion':'default-1','canManage':False}}
            ...
            async def route(r):
                ...
                if '/api/' in r.request.url:
                    if r.request.method=='PUT':
                        ...
                        if r.request.url.endswith('/my-default'): state['myDefault'].update(enabled=data['enabled'],rowVersion='pm-default-2')
                        elif r.request.url.endswith('/default'): state['defaults'].update(enabled=data['enabled'],rowVersion='default-2')
            ...
            toggle=page.get_by_role('checkbox',name='Automatically create the first AI plan for this project')
            await toggle.wait_for();assert await toggle.is_enabled()
            assert await page.get_by_text('Organization default for administrators').count()==0
            await toggle.click()
            ...
            state.update(canManage=False);state['myDefault']['canManage']=False;state['defaults']['canManage']=False
            await page.get_by_role('button',name='Refresh status').click()
            await page.get_by_text('Your assigned PM, PM lead, or administrator can change this project setting.').wait_for()
            ...
            state.update(canManage=True,status='ready_for_review',message='Your first AI draft is ready for review.',completedAt='2026-09-20T12:01:00Z',
                         phases=[{'name':name,'number':i+1,'status':'completed','startedAt':'2026-09-20T12:00:00Z','completedAt':'2026-09-20T12:00:10Z','taskCount':3} for i,name in enumerate(['Plan','Design','Implement','Validate','Release'])]);state['myDefault']['canManage']=True;state['defaults']['canManage']=True
            await page.get_by_role('button',name='Refresh status').click()
            await page.get_by_role('button',name='Review working draft').click()
            assert await page.evaluate('window.draftLoads')==1
            personal=page.get_by_role('checkbox',name='Use automatic planning for new projects assigned to me')
            await personal.click()
            await expect(personal).to_be_checked()
            assert len(writes)==2 and writes[-1][0].endswith('/my-default') and writes[-1][1]['expectedVersion']=='pm-default-1'
            await page.get_by_text('Organization default for administrators',exact=True).click()
            organization=page.get_by_role('checkbox',name='Enable automatic planning for new projects when no PM preference is set')
            await organization.click()
            await expect(organization).to_be_checked()
            assert len(writes)==3 and writes[-1][0].endswith('/default') and writes[-1][1]['expectedVersion']=='default-1'
            ...
            hold=True
            await page.get_by_role('button',name='Refresh status').click();await held.wait()
