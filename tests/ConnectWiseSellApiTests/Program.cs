...
Check(ConnectWiseSellApi.TryPackCredential("tenant_azure", "public-test", "private-test", out var secret), "three valid keys accepted");
using var request = new HttpRequestMessage(HttpMethod.Get, ConnectWiseSellApi.HealthUrl);
Check(ConnectWiseSellApi.TryAuthorize(request, secret), "request authenticated");
...
foreach (var uri in new[] { "https://api.getbase.com/v2/contacts", "https://api-na.myconnectwise.net/v4_6_release/apis/3.0/company/companies", "https://evil.example/api/quotes", "http://sellapi.quosalsell.com/api/quotes", "https://sellapi.quosalsell.com.evil.example/api/quotes", "https://sellapi.quosalsell.com:8443/api/quotes", "https://user@sellapi.quosalsell.com/api/quotes", "https://sellapi.quosalsell.com/api/quotes-malicious" })
{
    ...
    Check(!ConnectWiseSellApi.TryAuthorize(wrong, secret) && wrong.Headers.Authorization is null, "reject credential destination " + uri);
}
foreach (var bad in new[] { "", "bearer-token", "{}", "{\"AccessKey\":\"a\"}" })
{
    using var invalid = new HttpRequestMessage(HttpMethod.Get, ConnectWiseSellApi.HealthUrl);
    Check(!ConnectWiseSellApi.TryAuthorize(invalid, bad), "reject malformed credential");
}
foreach (var bad in new[] { "", "a:b", "a+b", "a\r\nb", "a b" })
    Check(!ConnectWiseSellApi.TryPackCredential(bad, "public", "private", out _), "reject malformed access key");
Check(!ConnectWiseSellApi.TryPackCredential("tenant", "public", "", out _), "private key required");
Check(ConnectWiseSellApi.IsQuoteList("[]"), "empty quote list is authenticated success");
Check(ConnectWiseSellApi.IsQuoteList("[{\"id\":\"quote-123\",\"name\":\"Sample\"}]"), "CPQ quote list accepted");
foreach (var body in new[] { "<html>Login</html>", "{\"items\":[{\"data\":{\"id\":123}}]}", "null", "[{}]", "[1]", "[{\"id\":\"\"}]" })
    Check(!ConnectWiseSellApi.IsQuoteList(body), "reject login, Zendesk or malformed response");
