...
async def main():
    ...
    try:
        ...
        def reset(deny=False):
            with urlopen(Request(runner.ORIGIN + '/__reset', method='POST',
                data=json.dumps({'status': 'confirmed', 'deniedModule': deny}).encode())) as response:
                response.read()
        ...
        print('MODULE025_REGISTER_BROWSER_REGRESSION=PASS full_app=true reads_only=true fixture_scoped=true')
    finally:
        server.terminate()
