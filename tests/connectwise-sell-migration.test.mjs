...
const authorityMigration = fs.readFileSync(new URL('../database/migrations/098_customer_directory_source_authority.sql', import.meta.url), 'utf8');
...
await db.exec(`
 ...
 CREATE TABLE crm_integration_providers (
  ...
  supports_attachments boolean, notes text, last_error_code text DEFAULT '', updated_at timestamptz DEFAULT now());
 CREATE TABLE crm_integration_oauth_states(provider_key text, used_at timestamptz);
 INSERT INTO crm_integration_oauth_states VALUES('zendesk_sell',null);
 ...
 CREATE TABLE customer_directory_sync_runs(source_system text);
...
assert.ok((await rows("SELECT used_at FROM crm_integration_oauth_states WHERE provider_key='zendesk_sell'"))[0].used_at);
...
const configuredAuthority = await rows('SELECT * FROM customer_directory_source_authority');
await db.exec(authorityMigration);
await db.exec(authorityMigration);
assert.deepEqual(await rows('SELECT * FROM customer_directory_source_authority'), configuredAuthority);
await db.exec(migration);
...
// If the singleton is missing after 111, restore the current vendor, never the retired one.
await db.exec('DELETE FROM customer_directory_source_authority');
await db.exec(authorityMigration);
assert.deepEqual(await rows('SELECT source_mode,provider_key FROM customer_directory_source_authority'), [{source_mode:'sell',provider_key:'connectwise_sell'}]);
await db.close();
