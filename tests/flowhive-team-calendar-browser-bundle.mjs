...
const result=await build({root,configFile:false,define:{'process.env.NODE_ENV':JSON.stringify('production')},plugins:[react(),{
  ...
  load(id){if(id==='\0'+entry)return `import React from ${JSON.stringify(path.join(root,'node_modules/react/index.js'))};import{createRoot}from ${JSON.stringify(path.join(root,'node_modules/react-dom/client.js'))};import Review from ${JSON.stringify(path.join(root,'src/ProjectFlowHiveTeamCalendar.jsx'))};import ${JSON.stringify(path.join(root,'src/project-flowhive-psa-workspace.css'))};const root=createRoot(document.getElementById('root'));window.mountFlowHiveTeamCalendar=props=>root.render(React.createElement(Review,props));`;}
