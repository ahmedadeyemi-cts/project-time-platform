...
test('protected cutover refresh uses a new approval reference and preserves historical evidence identity', () => {
  const authorization = JSON.parse(fs.readFileSync(new URL('../.github/flowhive-psa-protected-cutover.json', import.meta.url), 'utf8'));
  assert.equal(authorization.approvalReference, 'FLOWHIVE-PSA-PROTECTED-CUTOVER-20260912-PLANNER-PHASE-ASSEMBLY-RELEASE');
  assert.equal(authorization.supersedesApprovalReference, 'FLOWHIVE-PSA-PROTECTED-CUTOVER-20260912-PLANNER-OUTPUT-SHAPE-RELEASE');
...
test('successor candidate binds to trusted main and rejects unincorporated application drift', () => {
  ...
  assert.equal(approval.pullRequest, 947);
  ...
  assert.equal(approval.mergeCommit, 'f8cf17a022b008f7fee81f7f2226e774c3056e69');
...
test('successor approval enumerates only the workflows that ran for the exact PR947 head', () => {
