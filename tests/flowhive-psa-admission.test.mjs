...
test('live planner candidate activation is bounded and otherwise remains inactive', () => {
  const authorization = JSON.parse(fs.readFileSync(new URL('../.github/flowhive-psa-protected-cutover.json', import.meta.url), 'utf8'));
  const activation = process.env.GITHUB_HEAD_REF === 'control/flowhive-live-planner-activation-20260912' || process.env.GITHUB_HEAD_REF === 'control/flowhive-planner-output-budget-final-activation-20260912' || process.env.GITHUB_HEAD_REF === 'control/flowhive-planner-latency-activation-20260912';
