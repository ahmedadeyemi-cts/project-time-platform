...
const connectWiseSellCorrection = process.env.GITHUB_HEAD_REF === 'fix/connectwise-sell-module026';
const module025RetainedLookup = process.env.GITHUB_HEAD_REF === 'fix/module025-retained-record-lookup-20260919';
const module025RetainedRegister = process.env.GITHUB_HEAD_REF === 'fix/module025-retained-register-verification-20260919';
const module025RegisterEntry = process.env.GITHUB_HEAD_REF === 'fix/module025-register-entry-diagnostics-20260919';
const module025RegisterStartup = process.env.GITHUB_HEAD_REF === 'fix/module025-register-startup-readiness-20260918';
const module025RegisterReport = process.env.GITHUB_HEAD_REF === 'fix/module025-register-report-contract-20260918';
const module025PhaseProviderRecovery = process.env.GITHUB_HEAD_REF === 'fix/module025-phase-provider-recovery-20260918';
const module025RegisterBrowser = process.env.GITHUB_HEAD_REF === 'fix/module025-register-browser-acceptance-20260918';
const module025WorkerAuthorization = process.env.GITHUB_HEAD_REF === 'fix/module025-fixture-generation-authorization-20260918';
const module025ReviewTimingDelete = process.env.GITHUB_HEAD_REF === 'feature/module025-review-timing-delete-reliability-20260918';
const module025AutoProtectedTest = process.env.GITHUB_HEAD_REF === 'feature/module025-auto-protected-test-20260917';
const module025ProjectName = process.env.GITHUB_HEAD_REF === 'feature/module025-project-name-20260917';
const module025ScopeProgress = process.env.GITHUB_HEAD_REF === 'fix/module025-scope-progress-20260917';
const moduleCatalogConsistency = process.env.GITHUB_HEAD_REF === 'fix/module025-catalog-consistency-20260917';
const module025WorkspaceEntry = process.env.GITHUB_HEAD_REF === 'fix/module025-workspace-entry-20260917';
const module025BrowserVerifier = process.env.GITHUB_HEAD_REF === 'fix/module025-browser-verifier-20260917';
const module025AssemblyPredecessors = process.env.GITHUB_HEAD_REF === 'fix/module025-assembly-predecessors-20260917';
...
const flowHiveMigration = process.env.GITHUB_HEAD_REF === 'fix/flowhive-migration-runner-20260920';
const flowHiveAutomatic = process.env.GITHUB_HEAD_REF === 'feature/flowhive-auto-first-draft-20260920';
const flowHiveSequential = process.env.GITHUB_HEAD_REF === 'feature/flowhive-sequential-wbs-20260920';
const flowHivePreparation = process.env.GITHUB_HEAD_REF === 'feature/flowhive-document-readiness-archive-20260920';
const flowHiveWorkspace = process.env.GITHUB_HEAD_REF === 'feature/flowhive-psa-team-workspace-20260919';
// This application preparation branch preserves the pinned release authority.
// Use the historical verifier fixture, never treat newer main as admitted drift.
const phaseProgressPreparation = process.env.GITHUB_HEAD_REF === 'feat/module025-phase-progress-20260920';
const saWorkspacePreparation = process.env.GITHUB_HEAD_REF === 'feat/sa-workspace-redesign-20260920';
...
const module025VerifierCorrection = module025ServiceScope || module064SequenceRepair || installedVerifierRepair || privateGenerationRepair || module064CatalogPreparation || flowHiveMigration || flowHiveAutomatic || flowHiveSequential || phaseProgressPreparation || saWorkspacePreparation || flowHivePreparation || flowHiveWorkspace || enterpriseCompletionCorrection || process.env.GITHUB_HEAD_REF === 'fix/module025-sa-review-confirm-20260916'
  || module025ExportRetention || module025StandardExports || privateGenerationCorrection || connectWiseSellCorrection || module025RetainedLookup || module025RetainedRegister || module025RegisterEntry || module025RegisterStartup || module025RegisterReport || module025PhaseProviderRecovery || module025RegisterBrowser || module025WorkerAuthorization || module025GenerationCorrection || module025ScopedDeployment || module025EngineRedesign || module025ExternalAdapter || module025ProviderDiagnostics || module025QualificationContract || module025QualificationPolicy || module025CompleteAcceptance || module025OutputBudgetPreflight || module025StructuredContract || module025VersionFacts || module025AssemblyPredecessors || module025BrowserVerifier || module025WorkspaceEntry || moduleCatalogConsistency || module025ScopeProgress || module025ProjectName || module025AutoProtectedTest || module025ReviewTimingDelete;
const module025VerifierBase = module025ServiceScope ? 'dc7297037c77a711276c7a36cd0048b4b54c6035' : module064SequenceRepair ? '96394a4a69da92190594a06467f69cf868639532' : installedVerifierRepair ? 'b693227f9ba0c483f5eb521f7fbe46e7ed697f3f' : privateGenerationRepair ? '219fad3923da2e08cdd1a84d54031d9dad76bf64' : module064CatalogPreparation ? 'a59e60eabf25c5a74d1075f5e189d9c5500f0a84' : flowHiveMigration ? '52fd4e2debf20bcbb34ea2f68199d4140aa01eea' : flowHiveAutomatic ? '045b66ca01baa68b2f5b3f6eb9e063c23c335981' : flowHiveSequential ? '3f55a0f62f44587d234a09a9f0ceabbe501da8c5' : phaseProgressPreparation ? '35a442bee3724d7d4e5865f4abc2aaa910d55326' : saWorkspacePreparation ? 'ad98f30d0a86f12b89c6fb2239ac9afcfd0dac2e' : flowHivePreparation ? '373b37e9c76e430355ed2dc5f71ec6a133ead3aa' : flowHiveWorkspace ? 'b86ccd29c8c191e1c578437a28e7695f8ccf702b' : enterpriseCompletionCorrection ? 'fc10591e4550243603b84af1a42153634228972d' : module025ExportRetention ? '45074ea00496ef561fda25e7a2f148280689ed97' : module025StandardExports ? 'fd38a04d2e0dc171bbf52c9066db7dc3d99e72b9' : privateGenerationCorrection ? 'a851956b6babaa085fe124ff17846bea922613e9' : connectWiseSellCorrection ? '79614e06d920c67c5a937bd6f64aa54223e4c256' : module025RetainedLookup ? '42cb8e43216264bd5876afd993c066cad5728c1a' : module025RetainedRegister ? '140f42205fbabb3452463fa66bd37d0d29a5a5aa' : module025RegisterEntry ? 'edf957df33b3cd22b91e20ca71f33c12811ceb72' : module025RegisterStartup ? 'ef0dd9cd7d64366d4115084646dea2dd9e412edd' : module025RegisterReport ? '382fa4a8182f24dacd0fe15a7bd1e465c8b0dccc' : module025PhaseProviderRecovery ? '66e373ab1b3005e3e542683261164f5915f170a0' : module025RegisterBrowser ? '5dac14686c3052b8053f5f67189e02f10f07408d' : module025WorkerAuthorization ? '734aa6d844a2ba2bc89e9f64d759db862ca4db58' : module025ReviewTimingDelete ? '7339b882ed97371b399ff8cd35b8e48f2bce3a6c' : module025AutoProtectedTest ? '73412b3a3509f2f4098d137748c4d376947c1143' : module025ProjectName ? '5c293fc66ecb50383361e1f6bdb2bcbd4cd11a7b' : module025ScopeProgress ? 'a98349acdd5cb6f4598fb3ead1ba96607210a405' : moduleCatalogConsistency ? '5480149fa33bcd60dd3d9ec30a88667114a301de' : module025WorkspaceEntry ? '8a0960ae12cff57e9a50bd5a33b42e777362d2a4' : module025BrowserVerifier ? '3b7185292c013cd032b87ce6764ea238813e9e7a' : module025AssemblyPredecessors ? '5205140a5b12cbb123cc3dff5ef4c8200e22e98c' : module025VersionFacts ? '8a8673d607554158710b897e888beeaa3188f577' : module025StructuredContract ? 'f30b4e3a8cc7ba8daba4cad001e5dca84f7140f6' : module025OutputBudgetPreflight ? '5f2f520890309db20d3897f56c09c1ae194a4e33' : module025CompleteAcceptance ? 'd53b9451d7b2eebda686e8bdb84b46d53dc9e7f9' : module025QualificationPolicy ? 'ee78efea179325e0ee4d8ef784c9415a28028ac0' : module025QualificationContract ? '0c1b3503759c27495476e06f1f285174e8dfb23b' : module025ProviderDiagnostics ? 'dd6403e4ba89a8d15fa6308b85a0d20994d6cd13' : module025ExternalAdapter ? 'e632c8d24549339a8476b9b50a8411b4d253ff02' : module025EngineRedesign ? '37dfd1554d50a7b45b25982faf9baa68603a0e43' : '2062979f3e95b17cf187642ec2e9eadb4f610758';
...
const plannerParallelPhaseCandidateRefresh = process.env.GITHUB_HEAD_REF === 'control/flowhive-planner-parallel-phase-candidate-refresh-20260915';
const plannerRuntimeCheckOmission = process.env.GITHUB_HEAD_REF === 'control/flowhive-planner-runtime-check-omission-20260915';
...
const plannerCandidateRefresh = process.env.GITHUB_HEAD_REF === 'control/flowhive-planner-live-completion-candidate-refresh-20260915';
const plannerAdmissionEvidenceCorrection = process.env.GITHUB_HEAD_REF === 'control/flowhive-planner-live-completion-admission-evidence-20260915';
...
const module025MyRoleLiveVerifier = process.env.GITHUB_HEAD_REF === 'fix/module025-my-role-live-verifier-20260914'
  ...
  || process.env.GITHUB_HEAD_REF === 'fix/module025-my-role-observability-20260914';
...
const dispatchRuns = workflowDispatchChecks.map(binding => ({ id: binding.runId, workflow_id: binding.workflowId ?? 0, path: binding.workflow,
...
test('protected cutover refresh uses a new approval reference and preserves historical evidence identity', () => {
  ...
  const plannerLiveCapacityCandidateRefresh = process.env.GITHUB_HEAD_REF === 'control/flowhive-planner-live-capacity-candidate-refresh-20260915'
    || process.env.GITHUB_HEAD_REF === 'fix/flowhive-portfolio-db-alias-20260915';
  assert.equal(authorization.approvalReference, module025CandidateRefresh
    ? 'FLOWHIVE-PSA-PROTECTED-CUTOVER-20260916-FLOWHIVE-PLANNER-LIVE-CELAR-COMPACT-PROMPT-01'
    ...
    : plannerCelarApprovalRefresh || plannerCelarRequiredCheckRefresh
    ? 'FLOWHIVE-PSA-PROTECTED-CUTOVER-20260915-FLOWHIVE-PLANNER-LIVE-CELAR-ACCEPTANCE-01'
    : plannerCelarApprovalRefresh || plannerCelarRequiredCheckRefresh
    ? 'FLOWHIVE-PSA-PROTECTED-CUTOVER-20260915-FLOWHIVE-PLANNER-LIVE-CELAR-ACCEPTANCE-01'
  ...
  assert.equal(authorization.supersedesApprovalReference, module025CandidateRefresh
    ? 'FLOWHIVE-PSA-PROTECTED-CUTOVER-20260916-FLOWHIVE-PLANNER-LIVE-CELAR-BUDGET-01'
    : providerContractRefresh
    ? 'FLOWHIVE-PSA-PROTECTED-CUTOVER-20260913-PLANNER-PROVIDER-BUDGET-FINAL-REFRESH-05'
    : finalRefresh
    ? 'FLOWHIVE-PSA-PROTECTED-CUTOVER-20260913-PLANNER-NATIVE-TEST-RENEWAL-04'
...
test('live planner candidate activation is bounded and otherwise remains inactive', () => {
  ...
  const nativeTestRequired = authorization.approval?.mode === 'native-test-environment';
  assert.equal(authorization.activationDecision, activation
    ? nativeTestRequired ? 'native-test-required' : 'approved'
    : 'hold');
...
test('trusted-main source drift starts at the approved application merge and rejects the old PR-base boundary', () => {
  ...
  const oldPrBaseChanges = execFileSync('git', ['diff', '--name-only', `${approval.sourceBase}..${currentMain}`], { encoding: 'utf8' })
    .split(/\r?\n/).filter(Boolean);
  assert.ok(oldPrBaseChanges.includes('src/backend/ProjectTime.Api/Ai/PulseAiPrivateRagService.cs'));
  assert.throws(() => verifySourceDrift(oldPrBaseChanges, manifest));
});
...
test('successor candidate binds to trusted main and rejects unincorporated application drift', () => {
  ...
  if (module025VerifierCorrection) {
    ...
    if (enterpriseCompletionCorrection) {
      ...
      execFileSync('python3', ['tests/enterprise-completion-scope.py']);
    }
    if (module025ProviderDiagnostics || module025CompleteAcceptance) {
  ...
  if (module025ManualTriggerExactCandidate) {
    ...
    assert.doesNotThrow(() => execFileSync('git', ['merge-base', '--is-ancestor', currentMain, candidateHead], { stdio: 'ignore' }));
    assert.throws(() => execFileSync('git', ['merge-base', '--is-ancestor', candidateHead, currentMain], { stdio: 'ignore' }));
    assert.notEqual(candidateHead, approval.sha, 'The control follow-up must not replace the approved application candidate.');
    assert.deepEqual(changedFiles, [
  ...
  if (module025AdmissionManifestRefresh) {
    const currentMain = execFileSync('git', ['rev-parse', 'origin/main'], { encoding: 'utf8' }).trim();
    const candidateHead = execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
    const changedFiles = execFileSync('git', ['diff', '--name-only', `${currentMain}...${candidateHead}`], { encoding: 'utf8' })
      .split(/\r?\n/).filter(Boolean).sort();
    assert.equal(currentMain, '557e32aaf2c6be34557990becc6d3dea092efc3a');
    assert.match(candidateHead, /^[0-9a-f]{40}$/);
    assert.doesNotThrow(() => execFileSync('git', ['merge-base', '--is-ancestor', currentMain, candidateHead], { stdio: 'ignore' }));
    assert.throws(() => execFileSync('git', ['merge-base', '--is-ancestor', candidateHead, currentMain], { stdio: 'ignore' }));
    assert.notEqual(candidateHead, approval.sha, 'The manifest correction must not replace the application candidate.');
    assert.deepEqual(changedFiles, module025AdmissionManifestRefreshFiles);
    return;
  }
  if (module025CandidateRefresh) {
    const currentMain = execFileSync('git', ['rev-parse', 'origin/main'], { encoding: 'utf8' }).trim();
    const candidateHead = execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
    const changedFiles = execFileSync('git', ['diff', '--name-only', `${currentMain}...${candidateHead}`], { encoding: 'utf8' })
      .split(/\r?\n/).filter(Boolean).sort();
    ...
    assert.doesNotThrow(() => execFileSync('git', ['merge-base', '--is-ancestor', currentMain, candidateHead], { stdio: 'ignore' }));
    assert.throws(() => execFileSync('git', ['merge-base', '--is-ancestor', candidateHead, currentMain], { stdio: 'ignore' }));
    assert.notEqual(candidateHead, approval.sha, 'The approval follow-up must not replace the application candidate.');
    assert.deepEqual(changedFiles, module025CandidateRefreshFiles);
    return;
  }
  if (plannerLiveCelarCompactPromptRepair) {
    const currentMain = execFileSync('git', ['rev-parse', 'origin/main'], { encoding: 'utf8' }).trim();
    const candidateHead = execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
    const changedFiles = execFileSync('git', ['diff', '--name-only', `${currentMain}...${candidateHead}`], { encoding: 'utf8' })
      .split(/\r?\n/).filter(Boolean).sort();
  ...
  const plannerAdmissionManifestRefresh = process.env.GITHUB_HEAD_REF === 'control/flowhive-planner-admission-manifest-refresh-20260915';
  const sourceDriftFiles = module025CandidateRefresh
    ? module025CandidateRefreshFiles
    : plannerLiveCelarBudgetApproval
    ? plannerLiveCelarBudgetApprovalFiles
    : plannerCelarApprovalRefresh || plannerCelarRequiredCheckRefresh
    ? plannerCelarApprovalRefreshFiles
    : plannerCandidateRefresh
    ? [...new Set([...files, ...plannerCandidateRefreshFiles])].sort()
    : portfolioDbAliasCandidateRefresh || plannerLiveCapacityCandidateRefresh
    ...
    : plannerParallelPhaseFix
    ? [...new Set([...files, ...plannerCompactPhaseCandidateRefreshFiles, ...plannerParallelPhaseFixFiles])].sort()
    : plannerProviderDeadlineCandidateRefresh
    ? [...new Set([...files, ...plannerProviderDeadlineCandidateRefreshFiles])].sort()
    : plannerCompactPhaseFix
    ? [...new Set([...files, ...plannerCompactPhaseFixFiles])].sort()
    : plannerProviderDeadlineRetry
    ? [...new Set([...files, ...triggerCoverageFiles, ...plannerProviderDeadlineRetryFiles])].sort()
    : triggerCoverage
    ? [...new Set([...files, ...triggerCoverageFiles])].sort()
    : module025SowRoleCandidateRefresh1014
    ? [...new Set([...files, ...module025SowRoleCandidateRefresh1014Files])].sort()
    : module025SowRoleCandidateRefreshFinal
    ...
    : plannerParallelPhaseCandidateRefresh
    ? [...new Set([...files, ...plannerParallelPhaseCandidateRefreshFiles])].sort()
  ...
  assert.equal(approval.mergeCommit, plannerCelarResponseContractRepair ? approval.mergeCommit : module025AdmissionManifestRefresh || module025CandidateRefresh || dispatchValidation ? 'b82bea8ad3874e58ab8d42f01017942fa9d8765e' : plannerLiveCelarBudgetApproval || plannerLiveCelarCompactPromptRepair || module025ManualTriggerExactCandidate ? '451432ac7e1dc922e699293d4f2e0ad1dfaa7f88' : plannerCelarApprovalRefresh || plannerCelarRequiredCheckRefresh ? 'f7ee256fb851cbdeb083c2ff6fe8ad650ee4d203' : plannerCandidateApprovalRefresh ? 'e15e6dcd872fe6e5eae790d2213dd0b8347e94f6' : plannerCandidateRefresh || plannerAdmissionEvidenceCorrection || portfolioDbAliasCandidateRefresh || plannerLiveCapacityCandidateRefresh || plannerLiveCapacityRepair || plannerParallelPhaseCandidateRefresh || plannerRuntimeCheckOmission || plannerCapacitySafeCandidateRefresh || plannerCapacitySafe
    ...
    : module025SowRoleCandidateRefresh1009 || module025MyRoleLiveVerifier
  ...
  if (module025CandidateRefresh || plannerLiveCelarBudgetApproval || plannerCelarApprovalRefresh || plannerCelarRequiredCheckRefresh) {
    assert.deepEqual(sourceDriftFiles, module025CandidateRefresh ? module025CandidateRefreshFiles : plannerLiveCelarBudgetApproval ? plannerLiveCelarBudgetApprovalFiles : plannerCelarApprovalRefreshFiles);
    ...
    ]) assert.throws(() => verifySourceDrift([...plannerCelarApprovalRefreshFiles, unrelated], sourceDriftFiles));
  } else {
...
test('successor approval enumerates only the workflows that ran for the exact selected application head', () => {
  const expectedRequiredWorkflows = plannerCelarRequiredCheckRefresh || plannerCelarApprovalRefresh || plannerCandidateApprovalRefresh ? [
...
test('workflow-dispatch evidence is candidate-bound and cannot substitute another run', () => {
  ...
  assert.equal(workflowDispatchChecks.length, module025AdmissionManifestRefresh || module025CandidateRefresh || dispatchValidation ? 3 : plannerLiveCelarBudgetApproval || plannerLiveCelarCompactPromptRepair || module025ManualTriggerExactCandidate || plannerCelarRequiredCheckRefresh ? 2 : plannerCelarApprovalRefresh || plannerCandidateApprovalRefresh ? 2 : 3);
...
test('path-filtered workflow omission is bound to the candidate inventory and base bytes', () => {
  ...
  if (approval.workflowPathOmissions.length === 0) {
    assert.deepEqual(approval.workflowPathOmissions, []);
    return;
  }
  ...
  assert.throws(() => verifyWorkflowPathOmission(pathMatchedOmission[0], pr, pathMatched, source),
