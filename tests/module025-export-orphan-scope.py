...
import subprocess
BASE = '57c8d0264bdd828e6b3b53a8c5cb8b1b841e8f61'
...
SELF = 'tests/module025-export-orphan-scope.py'
...
a=s.index('            # Exact export-release orphan:')
b=s.index('            run_status=',a)
block=s[a:b]
for required in ['git merge-base --is-ancestor', 'git diff --quiet', 'verify_zero_job_quarantine', 'pending_deployments', 'type == "array" and length == 0', BASE, '2026-09-19T18:30:44Z']:
    assert required in block
s=(s[:a]+s[b:]).replace("      QUARANTINED_ZERO_JOB_RUN_ID_6: '35461429670'\n",'')
assert s == show(SUP), 'All other admission/dispatch/verification logic must remain identical'
s=Path(REG).read_text()
added='if [[ "$HEAD_BRANCH" == fix/module025-export-orphan-recovery ]]; then\n  python3 tests/module025-export-orphan-scope.py\n  node tests/module025-deployment-startup-recovery.test.mjs\n  node tests/validate-systemwide-image-build-controller.mjs\nelif'
...
print('MODULE025_EXPORT_ORPHAN_SCOPE=PASS deployment_controller=unchanged production=unchanged')
