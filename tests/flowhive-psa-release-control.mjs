...
export function verifyFiles(changed, manifest, mode = 'initial', context = null) {
  ...
  assert.ok(['initial','pr874-digest-repair','reviewed-regeneration-105','candidate-refresh','successor-candidate-refresh','source-base-correction','successor-approval','successor-release-approval','successor-cutover-activation','dispatch-run-recovery','stale-run-supersession','stale-run-activation','stale-run-renewal','migration106-wiring','release-stabilization','protected-cutover','protected-cutover-activation','protected-cutover-refresh','admission-permission-fix','admission-bot-claim-fix','protected-cutover-recovery','reservation-recovery-endpoint-fix','protected-cutover-final','live-uat-status-fix','live-uat-pm-secret-wiring','installed-verification-environment','installed-verification-evidence','installed-acceptance-evidence','installed-acceptance-artifacts','installed-acceptance-readiness','installed-acceptance-visibility','installed-acceptance-planner','installed-acceptance-final'].includes(mode), 'Unrecognized control repair.');
...
export function verifyController(text) {
  for (const token of [
    ...
    'run-flowhive-psa-live-uat.py', 'PROJECTPULSE_M025_PM_EMAIL: ${{ secrets.PROJECTPULSE_M025_PM_EMAIL }}', 'PROJECTPULSE_M025_PM_PASSWORD: ${{ secrets.PROJECTPULSE_M025_PM_PASSWORD }}', 'RELIABILITY_RELEASE_COMMIT:', 'Seal server-confirmed deployment identity',
...
export function validate() {
  ...
  const isLiveUatPmSecretWiring = process.env.GITHUB_HEAD_REF === liveUatPmSecretWiringBranch;
