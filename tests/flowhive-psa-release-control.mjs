...
export function validate() {
  ...
  if (isProtectedCutover || isProtectedCutoverActivation || isProtectedCutoverRefresh || isProtectedCutoverRecovery || isProtectedCutoverFinal || isSuccessorReleaseApproval || isSuccessorCutoverActivation || isPlannerOutputShapeApproval || isPlannerTimeBudgetApproval || isPlannerProviderBudgetSuccessorApproval || isPlannerProviderBudgetActivation || isPlannerTimeBudgetActivation || isPlannerOutputBudgetActivation || isPlannerOutputBudgetActivationRefresh || isDispatchSkippedRecovery) {
    if (isProtectedCutoverActivation || isProtectedCutoverRefresh || isProtectedCutoverRecovery || isProtectedCutoverFinal || isSuccessorCutoverActivation || isPlannerProviderBudgetActivation || isPlannerTimeBudgetActivation || isPlannerOutputBudgetActivation || isPlannerOutputBudgetActivationRefresh || isDispatchSkippedRecovery) {
      ...
      assert.equal(protectedCutover.workflow.allowControllerActivation, isPlannerProviderBudgetActivation || isPlannerTimeBudgetActivation || isSuccessorCutoverActivation || isPlannerOutputBudgetActivation || isPlannerOutputBudgetActivationRefresh || isDispatchSkippedRecovery || isProtectedCutoverLaneRefresh || isProtectedCutoverPmIdentityRefresh ? false : true);
    ...
    if (isProtectedCutover || isSuccessorReleaseApproval || isPlannerOutputShapeApproval || isPlannerTimeBudgetApproval || isPlannerProviderBudgetSuccessorApproval) {
