...
export const registerSowRoleAcceptanceControlBase = 'be8f883c452bb190cf43ced3142df03e079879dc';
export const registerSowRoleAcceptanceControlBranch = 'control/register-sow-role-acceptance-ci-20260913';
export const registerSowRoleAcceptanceControlFiles = [
  '.github/flowhive-psa-release-control-files.txt',
  '.github/workflows/flowhive-psa-release-control-ci.yml',
  'tests/flowhive-psa-release-control.mjs'
].sort();
...
export function verifyFiles(changed, manifest, mode = 'initial', context = null) {
  ...
  assert.ok(['initial','register-sow-role-acceptance-control','pr874-digest-repair','reviewed-regeneration-105','candidate-refresh','successor-candidate-refresh','source-base-correction','successor-approval','successor-release-approval','successor-cutover-activation','planner-output-budget-approval','planner-output-shape-approval','planner-output-shape-activation','planner-output-shape-branch-correction','planner-phase-candidate-refresh','live-planner-candidate-refresh','planner-latency-candidate-refresh','planner-context-budget-candidate-refresh','planner-single-batch-candidate-refresh','planner-compact-batch-candidate-refresh','planner-compact-batch-activation','planner-compact-batch-admission-ancestry-fix','planner-compact-batch-source-drift-boundary','planner-compact-batch-activation-renewal','planner-live-repair-candidate-refresh','planner-live-repair-final-refresh','planner-provider-contract-refresh','planner-live-provider-output-refresh','planner-native-test-approval-renewal','planner-single-batch-activation','planner-context-budget-activation','planner-latency-activation','live-planner-activation','planner-time-budget-approval','planner-provider-budget-successor-approval','planner-provider-budget-activation','planner-time-budget-workflow-set','planner-time-budget-activation','planner-output-budget-activation','planner-output-budget-activation-refresh','planner-output-budget-candidate-refresh','planner-output-budget-final-activation','dispatch-run-recovery','dispatch-lane-repair','dispatch-skipped-recovery','dispatch-receipt-parser','dispatch-lane-admission','stale-run-supersession','stale-run-activation','stale-run-renewal','migration106-wiring','release-stabilization','protected-cutover','protected-cutover-activation','protected-cutover-refresh','protected-cutover-lane-refresh','protected-cutover-pm-identity-refresh','admission-permission-fix','admission-bot-claim-fix','protected-cutover-recovery','reservation-recovery-endpoint-fix','protected-cutover-final','live-uat-status-fix','installed-verification-environment','installed-verification-evidence','installed-acceptance-evidence','installed-acceptance-artifacts','installed-acceptance-readiness','installed-acceptance-visibility','installed-acceptance-planner','installed-acceptance-final'].includes(mode), 'Unrecognized control repair.');
  if (mode === 'register-sow-role-acceptance-control') {
    assert.equal(context?.base, registerSowRoleAcceptanceControlBase, 'SOW role acceptance control registration must be based on trusted main.');
    assert.equal(context?.branch, registerSowRoleAcceptanceControlBranch, 'Wrong SOW role acceptance control branch.');
  }
  ...
  const effectiveScopedExpected = mode === 'planner-native-test-approval-renewal' ? plannerNativeTestApprovalRenewalFiles : mode === 'planner-live-repair-candidate-refresh' ? plannerLiveRepairCandidateRefreshFiles : mode === 'planner-live-repair-final-refresh' ? plannerLiveRepairFinalRefreshFiles : mode === 'planner-provider-contract-refresh' ? plannerProviderContractRefreshFiles : mode === 'planner-live-provider-output-refresh' ? plannerLiveProviderOutputRefreshFiles : scopedExpected;
  const modeExpected = mode === 'register-sow-role-acceptance-control' ? registerSowRoleAcceptanceControlFiles : effectiveScopedExpected;
  assert.deepEqual([...changed].sort(), modeExpected, 'Unexpected or missing file in the release-control PR.');
}
...
export function validate() {
  ...
  const isRegisterSowRoleAcceptanceControl = process.env.GITHUB_HEAD_REF === registerSowRoleAcceptanceControlBranch;
  if (isRegisterSowRoleAcceptanceControl) verifyFiles(changed, manifest, 'register-sow-role-acceptance-control', context);
  ...
  if (isProtectedCutover || isProtectedCutoverActivation || isProtectedCutoverRefresh || isProtectedCutoverRecovery || isProtectedCutoverFinal || isSuccessorReleaseApproval || isSuccessorCutoverActivation || isPlannerOutputShapeApproval || isPlannerOutputShapeActivation || isPlannerOutputShapeBranchCorrection || isPlannerPhaseCandidateRefresh || isLivePlannerCandidateRefresh || isPlannerLatencyCandidateRefresh || isPlannerContextBudgetCandidateRefresh || isPlannerSingleBatchCandidateRefresh || isPlannerCompactBatchCandidateRefresh || isPlannerCompactBatchActivation || isPlannerCompactBatchAdmissionAncestryFix || isPlannerCompactBatchSourceDriftBoundary || isPlannerCompactBatchActivationRenewal || isPlannerSingleBatchActivation || isPlannerContextBudgetActivation || isPlannerLatencyActivation || isLivePlannerActivation || isPlannerTimeBudgetApproval || isPlannerProviderBudgetSuccessorApproval || isPlannerProviderBudgetActivation || isPlannerTimeBudgetActivation || isPlannerOutputBudgetActivation || isPlannerOutputBudgetActivationRefresh || isPlannerOutputBudgetCandidateRefresh || isPlannerOutputBudgetFinalActivation || isPlannerLiveRepairFinalRefresh || isPlannerProviderContractRefresh || isPlannerLiveProviderOutputRefresh || isDispatchSkippedRecovery) {
    ...
    } else if (isPlannerLiveRepairFinalRefresh || isPlannerProviderContractRefresh || isPlannerLiveProviderOutputRefresh) {
