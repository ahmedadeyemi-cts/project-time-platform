...
export function verifyFiles(changed, manifest, mode = 'initial', context = null) {
  ...
  const effectiveScopedExpected = mode === 'planner-native-test-approval-renewal' ? plannerNativeTestApprovalRenewalFiles : mode === 'planner-live-repair-candidate-refresh' ? plannerLiveRepairCandidateRefreshFiles : mode === 'planner-live-repair-final-refresh' ? plannerLiveRepairFinalRefreshFiles : mode === 'planner-provider-contract-refresh' ? plannerProviderContractRefreshFiles : mode === 'planner-live-provider-output-refresh' ? plannerLiveProviderOutputRefreshFiles : mode === 'installed-verifier-main-path' ? installedVerifierMainPathFiles : scopedExpected;
...
export function validate() {
  ...
  if (!isPlannerNativeTestApprovalRenewal && !isPlannerLiveRepairCandidateRefresh && !isPlannerLiveRepairFinalRefresh && !isPlannerProviderContractRefresh && !isPlannerLiveProviderOutputRefresh && !isInstalledVerifierMainPath) verifyFiles(changed, manifest,
