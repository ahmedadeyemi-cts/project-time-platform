...
export const plannerNativeTestApprovalRenewalBranch = 'control/flowhive-planner-live-repair-renewal-20260913';
...
export function verifyFiles(changed, manifest, mode = 'initial', context = null) {
  ...
  const effectiveScopedExpected = mode === 'planner-native-test-approval-renewal' ? plannerNativeTestApprovalRenewalFiles : mode === 'planner-live-repair-candidate-refresh' ? plannerLiveRepairCandidateRefreshFiles : scopedExpected;
...
export function validate() {
  ...
  if (!isPlannerNativeTestApprovalRenewal && !isPlannerLiveRepairCandidateRefresh) verifyFiles(changed, manifest,
  ...
  assert.ok(!/azure\/login|id-token:|contents:\s*write/.test(supervisor), 'Admission cannot mutate a cloud environment or source.');
  assert.match(supervisor, /environment:\s*test/, 'Admission must be held by the native Test environment barrier.');
