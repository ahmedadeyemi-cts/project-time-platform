...
assert_eq 0 "$(value "SELECT COUNT(*) FROM scoped_role_policy_effective_grants WHERE role_code IN ('PROJECT_MANAGEMENT','PROJECT_MANAGEMENT_LEAD') AND action_code ILIKE '%PASSWORD%' AND grant_effect='GRANT';")" pm_password_reset_not_inherited
