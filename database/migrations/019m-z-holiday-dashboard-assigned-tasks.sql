...
-- Ensure only Administrator and Project Team Coordinator can manage/upload holidays.
...
-- Project Info / Project Allocation should stay away from Engineers and PMs.
DELETE FROM app_role_permissions rp
USING app_roles r, app_permissions p
WHERE rp.app_role_id = r.app_role_id
  AND rp.app_permission_id = p.app_permission_id
...
-- Assignment hours foundation for workspace and timesheet available tasks.
...
GRANT SELECT ON TABLE projects, project_tasks, clients, app_users, time_entries TO "ptp_app";

-- Backfill assignment hours from resource request assignment allocations where available.
