...
CREATE TABLE IF NOT EXISTS role_access_repair_062_permission_changes (
...
CREATE TRIGGER trg_role_access_repair_062_assignments_immutable
BEFORE UPDATE OR DELETE ON role_access_repair_062_assignment_changes
...
CREATE TRIGGER trg_role_access_repair_062_permissions_immutable
BEFORE UPDATE OR DELETE ON role_access_repair_062_permission_changes
FOR EACH ROW EXECUTE FUNCTION projectpulse_062_block_evidence_mutation();
...
SELECT
    ...
    'Migration 062 canonical Super Administrator reconciliation',
    TRUE,
    NOW(),
    NOW()
...
INSERT INTO role_access_repair_062_permission_changes (
    role_id,
    permission_id,
    role_code,
    permission_code
...
SELECT
    ...
    NOW()
FROM role_access_repair_062_permission_changes change
...
BEGIN
    ...
    IF missing_permission_relationships <> 0 THEN
        RAISE EXCEPTION 'Migration 062 invariant failed: % administrator permission relationships are missing.', missing_permission_relationships;
    END IF;
END;
$assert_invariants$;
