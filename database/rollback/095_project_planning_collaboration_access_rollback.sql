...
-- Preserve the exact set of permissions created by 095 before dropping the
-- evidence tables that hold RESTRICT foreign keys back to app_permissions.
CREATE TEMP TABLE project_planning_095_permissions_to_remove
ON COMMIT DROP
AS
SELECT app_permission_id, permission_code
FROM project_planning_095_permissions_created;
...
-- These evidence tables reference app_permissions with ON DELETE RESTRICT, so
-- they must be removed before deleting the permissions they recorded.
...
USING project_planning_095_permissions_to_remove evidence
