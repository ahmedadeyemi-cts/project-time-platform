-- Guarded rollback for Pulse migration 075.
-- Existing PULSE invoice identities remain valid and immutable. Only the
-- generator for future invoices returns to the legacy prefix.

BEGIN;

DO $pulse075_rollback_prerequisites$
BEGIN
    IF to_regclass('public.schema_migrations') IS NULL
       ...
       OR to_regclass('public.billing_invoice_series_seq') IS NULL THEN
        RAISE EXCEPTION 'Migration 075 rollback requires the billing integration foundation.';
    END IF;
END;
$pulse075_rollback_prerequisites$;
...
BEGIN
    ...
    INSERT INTO project_billing_profiles (project_id, invoice_series_number)
    VALUES (p_project_id, nextval('billing_invoice_series_seq'))
