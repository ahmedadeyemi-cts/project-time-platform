...
BEGIN
  ...
  FOR i IN 1..10 LOOP
    task:=task_ids[i];
    tjson:=task_template||jsonb_build_object('task_id',task,'project_id',CASE WHEN i<=5 THEN q2 ELSE july END,'task_code',CASE WHEN i<=5 THEN format('DEMO-Q2-%s',lpad(i::text,2,'0')) ELSE format('DEMO-JUL-%s',lpad((i-5)::text,2,'0')) END,'task_name',names[i],'description',descriptions[i],'status',CASE WHEN i<=5 THEN 'closed' ELSE 'active' END,'billable',true,'is_active',true,'start_date',CASE WHEN i<=5 THEN '2026-04-01'::date ELSE '2026-07-01'::date END,'end_date',CASE WHEN i<=5 THEN '2026-06-30'::date ELSE '2026-12-31'::date END,'estimated_hours',CASE WHEN i<=5 THEN 72.8 ELSE 40 END,'allocated_hours',CASE WHEN i<=5 THEN 72.8 ELSE 40 END,'created_at',now(),'updated_at',now());
    INSERT INTO project_tasks SELECT (jsonb_populate_record(NULL::project_tasks,tjson)).*;
    ajson:=assignment_template||jsonb_build_object('project_assignment_id',(md5('DEMO-CUCM-ASSIGN-'||i::text))::uuid,'project_id',CASE WHEN i<=5 THEN q2 ELSE july END,'task_id',task,'user_id',kevin,'assigned_by_user_id',actor,'assignment_role','engineer','role_code','ENGINEERING','allocation_percentage',CASE WHEN i<=5 THEN 70 ELSE 100 END,'allocated_hours',CASE WHEN i<=5 THEN 72.8 ELSE 40 END,'start_date',CASE WHEN i<=5 THEN '2026-04-01'::date ELSE '2026-07-01'::date END,'end_date',CASE WHEN i<=5 THEN '2026-06-30'::date ELSE '2026-12-31'::date END,'status',CASE WHEN i<=5 THEN 'closed' ELSE 'active' END,'is_active',CASE WHEN i<=5 THEN false ELSE true END,'created_at',now(),'updated_at',now());
