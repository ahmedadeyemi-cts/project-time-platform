-- 055D.4I
-- Allow non-login @ussignal.cloud placeholder users only for Work Register intake stakeholders.
-- Do not allow @ussignal.com manual users. Those still must come from Entra.
...
BEGIN
    ...
    -- Real cloud/domain users must come from Entra.
    ...
    -- Allow only non-login ProjectPulse placeholder stakeholders.
    IF v_email LIKE '%@ussignal.cloud' THEN
        IF v_source_provider = 'work_register_intake_temp_cloud'
        ...
        RAISE EXCEPTION 'Manual @ussignal.cloud users are only allowed as non-login Work Register intake placeholder stakeholders.';
    END IF;

    -- Existing local/dev behavior remains allowed.
...
DECLARE
    ...
    v_effective_job_title text;
    v_effective_team_name text;
BEGIN
    ...
    v_effective_job_title := v_effective_role_name;
    v_effective_team_name := v_effective_role_name;
    ...
    IF v_user_id IS NULL THEN
        ...
        VALUES (
            ...
            v_display_name,
            NULL,
            v_effective_job_title,
            v_effective_team_name,
            TRUE,
            NOW(),
            NOW(),
    ...
    ELSE
        ...
        SET display_name = COALESCE(NULLIF(display_name, ''), v_display_name),
            job_title = COALESCE(NULLIF(job_title, ''), v_effective_job_title),
            department = COALESCE(NULLIF(department, ''), v_effective_team_name),
            department_name = COALESCE(NULLIF(department_name, ''), v_effective_team_name),
            team_name = COALESCE(NULLIF(team_name, ''), v_effective_team_name),
            is_active = TRUE,
            login_enabled = CASE
    ...
    IF NOT EXISTS (
        ...
        VALUES (
            ...
            v_app_role_id,
            p_actor_user_id,
            'Automatic role assigned by Work Register intake final save.',
            TRUE,
            NOW(),
            NOW()
        );
