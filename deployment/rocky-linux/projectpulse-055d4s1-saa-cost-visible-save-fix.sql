...
CREATE OR REPLACE FUNCTION pp055d4s1_text(payload jsonb, key_name text)
RETURNS text
LANGUAGE plpgsql
AS $fn$
DECLARE
...
CREATE OR REPLACE FUNCTION pp055d4s1_uuid(value text)
RETURNS uuid
LANGUAGE plpgsql
AS $fn$
BEGIN
...
CREATE OR REPLACE FUNCTION pp055d4s1_money(value text)
RETURNS numeric
LANGUAGE plpgsql
AS $fn$
DECLARE
...
CREATE OR REPLACE FUNCTION pp055d4s1_contract(value text)
RETURNS text
LANGUAGE sql
AS $fn$
    SELECT CASE
...
CREATE OR REPLACE FUNCTION pp055d4s1_upsert_stakeholder(
    p_project_id uuid,
    p_role text,
    p_user_id uuid,
...
DECLARE
    ...
    v_salesforce text;
BEGIN
    v_project_id := pp055d4s1_uuid(coalesce(
        pp055d4s1_text(p_payload, 'projectId'),
        pp055d4s1_text(p_payload, 'project_id'),
        pp055d4s1_text(p_payload, 'workId'),
        pp055d4s1_text(p_payload, 'work_id'),
        pp055d4s1_text(p_payload, 'id')
    ));
    ...
    v_contract_type := pp055d4s1_contract(coalesce(
        pp055d4s1_text(p_payload, 'contractType'),
        pp055d4s1_text(p_payload, 'contract_type'),
        pp055d4s1_text(p_payload, 'contract')
    ));

    v_saa_user_id := pp055d4s1_uuid(coalesce(
        pp055d4s1_text(p_payload, 'insideSalesUserId'),
        pp055d4s1_text(p_payload, 'inside_sales_user_id'),
        pp055d4s1_text(p_payload, 'saaUserId'),
        pp055d4s1_text(p_payload, 'solutionArchitectAssociateUserId'),
        pp055d4s1_text(p_payload, 'solution_architect_associate_user_id')
    ));

    v_pm_user_id := pp055d4s1_uuid(coalesce(
        pp055d4s1_text(p_payload, 'projectManagerUserId'),
        pp055d4s1_text(p_payload, 'pmUserId'),
        pp055d4s1_text(p_payload, 'project_manager_user_id')
    ));

    v_pc_user_id := pp055d4s1_uuid(coalesce(
        pp055d4s1_text(p_payload, 'projectCoordinatorUserId'),
        pp055d4s1_text(p_payload, 'projectTeamCoordinatorUserId'),
        pp055d4s1_text(p_payload, 'pcUserId'),
        pp055d4s1_text(p_payload, 'ptcUserId')
    ));

    v_ae_user_id := pp055d4s1_uuid(coalesce(
        pp055d4s1_text(p_payload, 'accountExecutiveUserId'),
        pp055d4s1_text(p_payload, 'aeUserId'),
        pp055d4s1_text(p_payload, 'account_executive_user_id')
    ));

    v_sa_user_id := pp055d4s1_uuid(coalesce(
        pp055d4s1_text(p_payload, 'solutionArchitectUserId'),
        pp055d4s1_text(p_payload, 'saUserId'),
        pp055d4s1_text(p_payload, 'solution_architect_user_id')
    ));

    v_total_cost := pp055d4s1_money(coalesce(
        pp055d4s1_text(p_payload, 'plannedTotalProjectCost'),
        pp055d4s1_text(p_payload, 'planned_total_project_cost'),
        pp055d4s1_text(p_payload, 'projectListPrice'),
        pp055d4s1_text(p_payload, 'project_list_price'),
        pp055d4s1_text(p_payload, 'totalCost'),
        pp055d4s1_text(p_payload, 'budget'),
        pp055d4s1_text(p_payload, 'contractValue'),
        pp055d4s1_text(p_payload, 'sellAmount')
    ));

    v_pm_cost := pp055d4s1_money(coalesce(
        pp055d4s1_text(p_payload, 'plannedPmCost'),
        pp055d4s1_text(p_payload, 'planned_pm_cost'),
        pp055d4s1_text(p_payload, 'pmCost')
    ));

    v_eng_cost := pp055d4s1_money(coalesce(
        pp055d4s1_text(p_payload, 'plannedEngineeringCost'),
        pp055d4s1_text(p_payload, 'planned_engineering_cost'),
        pp055d4s1_text(p_payload, 'engineeringCost')
    ));

    v_quote := coalesce(
        pp055d4s1_text(p_payload, 'sellQuoteNumber'),
        pp055d4s1_text(p_payload, 'sell_quote_number'),
        pp055d4s1_text(p_payload, 'sellQuote'),
        pp055d4s1_text(p_payload, 'quoteNumber'),
        ''
    );

    v_salesforce := coalesce(
        pp055d4s1_text(p_payload, 'salesforceIdNumber'),
        pp055d4s1_text(p_payload, 'salesforce_id_number'),
        pp055d4s1_text(p_payload, 'salesforceId'),
        pp055d4s1_text(p_payload, 'sfId'),
        pp055d4s1_text(p_payload, 'opportunityId'),
        ''
    );
    ...
    IF v_saa_user_id IS NOT NULL THEN
        PERFORM pp055d4s1_upsert_stakeholder(v_project_id, 'SAA', v_saa_user_id, p_actor_user_id);
        PERFORM pp055d4s1_upsert_stakeholder(v_project_id, 'Inside Sales', v_saa_user_id, p_actor_user_id);
        PERFORM pp055d4s1_upsert_stakeholder(v_project_id, 'Inside Sales / SAA', v_saa_user_id, p_actor_user_id);
        PERFORM pp055d4s1_upsert_stakeholder(v_project_id, 'Solution Architect Associate', v_saa_user_id, p_actor_user_id);
    END IF;

    IF v_pc_user_id IS NOT NULL THEN
        PERFORM pp055d4s1_upsert_stakeholder(v_project_id, 'Project Team Coordinator', v_pc_user_id, p_actor_user_id);
        PERFORM pp055d4s1_upsert_stakeholder(v_project_id, 'Project Coordinator', v_pc_user_id, p_actor_user_id);
    END IF;
