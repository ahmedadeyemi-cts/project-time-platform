...
MIGRATION_FILE="$REPO_DIR/database/migrations/008_manager_approval_day_fields.sql"
MIGRATION_ID="008_manager_approval_day_fields"
...
echo "==> Validating manager approval columns"
PGPASSWORD="$PTP_DB_PASSWORD" psql \
  -h "$PTP_DB_HOST" \
  -p "$PTP_DB_PORT" \
  -U "$PTP_DB_USER" \
  -d "$PTP_DB_NAME" \
  -c "SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name = 'timesheet_day_statuses' AND column_name LIKE 'manager_%' ORDER BY ordinal_position;"
