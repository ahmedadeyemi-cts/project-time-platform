...
CREATE OR REPLACE FUNCTION projectpulse055d4c_numeric_or_zero(value text)
RETURNS numeric
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION projectpulse055d4c_bool_or_default(value text, default_value boolean)
RETURNS boolean
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION projectpulse055d4c_uuid_or_null(value text)
RETURNS uuid
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION projectpulse055d4c_email_from_name(display_name text)
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION projectpulse055d4c_existing_source_provider()
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION projectpulse055d4c_get_or_create_stakeholder_user(
    p_display_name text,
    p_role_code text,
    p_role_name text,
    p_job_title text,
...
BEGIN
    ...
    v_email := projectpulse055d4c_email_from_name(v_display_name);
    ...
    IF v_user_id IS NULL THEN
        ...
        VALUES (
            v_user_id, NULL, v_email, v_display_name, NULL,
            p_job_title, p_team_name, TRUE, NOW(), NOW(),
            projectpulse055d4c_existing_source_provider(), p_team_name, NULL, NULL,
            FALSE, p_team_name
        );
    ELSE
        UPDATE app_users
...
CREATE OR REPLACE FUNCTION projectpulse055d4c_commit_intake_package(
...
BEGIN
    ...
    v_customer_id := coalesce(projectpulse055d4c_uuid_or_null(v_review->>'customerId'), v_pkg.customer_id);
    ...
    v_pm_user_id := projectpulse055d4c_uuid_or_null(v_review #>> '{assignmentPlan,projectManagerUserId}');
    v_pc_user_id := projectpulse055d4c_uuid_or_null(v_review #>> '{assignmentPlan,projectCoordinatorUserId}');

    v_ae_user_id := projectpulse055d4c_get_or_create_stakeholder_user(v_review->>'accountExecutiveName', 'ACCOUNT_EXECUTIVE', 'Account Executive', 'Account Executive', 'Sales', p_actor_user_id);
    v_sa_user_id := projectpulse055d4c_get_or_create_stakeholder_user(v_review->>'solutionArchitectName', 'SOLUTION_ARCHITECT', 'Solution Architect', 'Solution Architect', 'Solution Architecture', p_actor_user_id);
    v_saa_user_id := projectpulse055d4c_get_or_create_stakeholder_user(v_review->>'insideSalesName', 'SOLUTION_ARCHITECT_ASSOCIATE', 'Solution Architect Associate', 'Solution Architect Associate', 'Solution Architecture', p_actor_user_id);

    SELECT COALESCE(SUM(projectpulse055d4c_numeric_or_zero(value->>'extendedAmount')), 0)
    INTO v_total_cost
    FROM jsonb_array_elements(CASE WHEN jsonb_typeof(v_review->'rates') = 'array' THEN v_review->'rates' ELSE '[]'::jsonb END)
    WHERE projectpulse055d4c_bool_or_default(value->>'include', TRUE) = TRUE;

    IF v_total_cost = 0 THEN
        v_total_cost := projectpulse055d4c_numeric_or_zero(v_review->>'projectListPrice');
    END IF;

    SELECT COALESCE(SUM(projectpulse055d4c_numeric_or_zero(value->>'laborListPrice')), 0)
    INTO v_pm_cost
    ...
    VALUES (
        ...
        v_total_cost,
        projectpulse055d4c_numeric_or_zero(v_review->>'pmHours'),
        projectpulse055d4c_numeric_or_zero(v_review->>'engineeringHours'),
        projectpulse055d4c_numeric_or_zero(v_review->>'travelHours'),
    ...
    BEGIN
        ...
        LOOP
            IF projectpulse055d4c_bool_or_default(v_rate.value->>'include', TRUE) = FALSE THEN CONTINUE; END IF;
            ...
            SELECT
                ...
                projectpulse055d4c_numeric_or_zero(v_rate.value->>'rate'),
    ...
    LOOP
        IF projectpulse055d4c_bool_or_default(v_task.value->>'include', TRUE) = FALSE THEN CONTINUE; END IF;
        ...
        VALUES (
            ...
            projectpulse055d4c_bool_or_default(v_task.value->>'billable', TRUE),
        ...
        LOOP
            IF projectpulse055d4c_uuid_or_null(v_assignment.value->>'engineerUserId') IS NULL THEN CONTINUE; END IF;
            ...
            VALUES (
                ...
                left(coalesce(NULLIF(v_task.value->>'taskName',''), NULLIF(v_task.value->>'phase',''), 'Task ' || v_task_idx::text), 240),
                projectpulse055d4c_uuid_or_null(v_assignment.value->>'engineerUserId'),
                NULL,
                projectpulse055d4c_numeric_or_zero(v_assignment.value->>'hours'),
                projectpulse055d4c_bool_or_default(v_task.value->>'billable', TRUE),
                ...
                NOW(),
                projectpulse055d4c_numeric_or_zero(v_assignment.value->>'allocationPercent'),
                'engineer', gen_random_uuid(),
                projectpulse055d4c_bool_or_default(v_assignment.value->>'isPrimary', v_assignment.ordinality = 1)
            );
