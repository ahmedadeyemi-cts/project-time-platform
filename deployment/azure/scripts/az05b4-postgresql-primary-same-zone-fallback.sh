...
LOG="$LOG_DIR/az05b4-postgresql-primary-$STAMP.log"
...
{
    section "AZ-05B4 - PostgreSQL Primary with Same-Zone HA Fallback"
    ...
    echo "Desired HA mode: ZoneRedundant"
    echo "Permitted fallback: SameZone"
    echo "TIME=$(date -u -Is)"

    section "Validating network foundation"
    ...
    echo "Reserved replica subnet: $SECONDARY_SUBNET_ID"
    echo "Private DNS zone: $PRIVATE_DNS_ZONE_ID"

    section "Validating Key Vaults"
    ...
    section "Retrieving existing administrator password"
    ...
    echo "Existing administrator password secret confirmed."
    ...
    if az postgres flexible-server show \
        ...
        echo "Existing PostgreSQL server confirmed: $PRIMARY_SERVER"
    else
        az postgres flexible-server create \
            ...
            --tags \
                ...
                "region-role=primary" \
                "desired-ha-mode=ZoneRedundant" \
                "same-zone-fallback=Allowed" \
    ...
    section "Waiting for PostgreSQL readiness"
    ...
    az postgres flexible-server parameter set \
        ...
        --only-show-errors \
        --output none ||
    echo "WARNING: Enhanced database activity metrics are unavailable."
    ...
    case "${HA_MODE,,}" in
        zoneredundant)
            echo "Zone-redundant HA was provisioned."
            ;;
        samezone)
            echo "WARNING: Azure used the permitted SameZone HA fallback."
            echo "This protects against node/service failure but not a complete zone outage."
            ;;
        *)
...
EOF
    ...
    section "AZ-05B4 completed successfully"
