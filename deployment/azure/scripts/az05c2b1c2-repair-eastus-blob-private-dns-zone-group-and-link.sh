...
LOG="$LOG_DIR/az05c2b1c2-repair-eastus-blob-private-dns-$STAMP.log"
WORK_DIR="$(mktemp -d)"
...
{
    section "AZ-05C2B1C2 - Repair East US Blob Private DNS Zone Group and VNet Link"
    ...
    [ -n "$PE_IP" ] || fail "Blob private endpoint IP is empty."
    [ "$PE_STATE" = "Succeeded" ] || fail "Blob private endpoint state is $PE_STATE."
    [ "$PE_CONNECTION" = "Approved" ] || fail "Blob private endpoint connection is $PE_CONNECTION."

    echo "BLOB_PRIVATE_DNS_ZONE_ID=$ZONE_ID"
...
PY
    ...
    if [ "$CORRECT_COUNT" = "0" ]; then
        BLOB_CONFIG_ID="$(python3 - "$ZONE_GROUP_JSON" "$ZONE_CONFIG_NAME" <<'PY'
import json
import sys
from pathlib import Path
...
for item in obj.get("privateDnsZoneConfigs") or []:
    if str(item.get("name") or "").lower() == wanted:
        print(str(item.get("privateDnsZoneId") or ""))
        break
PY
)"
    ...
    echo "BLOB_DNS_ZONE_GROUP_ACTION=$ZONE_GROUP_ACTION"
    echo "BLOB_DNS_ZONE_GROUP_STATE=$ZONE_GROUP_STATE"
    echo "BLOB_DNS_ZONE_GROUP_ASSOCIATION=confirmed"
    ...
    echo "BLOB_DNS_LINK_NAME=$EFFECTIVE_LINK"
    ...
    for attempt in 1 2 3 4 5 6; do
        ...
        echo "Waiting for private DNS A record update (attempt $attempt/6)."
        sleep 5
    done

    [ -n "$RECORD_IPS" ] || fail "Storage private DNS A record is missing."
    grep -Fxq "$PE_IP" <<< "$RECORD_IPS" || fail "Storage A record does not contain $PE_IP."
    ...
    section "AZ-05C2B1C2 complete"

    echo "No VM, database, role assignment, or storage data was modified."
    echo
    echo "************************************************************"
    echo "EASTUS BLOB PRIVATE DNS ZONE GROUP AND LINK READY"
    echo "************************************************************"
