...
function PrivateRagActivation({ rag, openKnowledge }) {
  const gates = [
    ['Private retrieval schema', rag?.schemaReady === true, 'Migrations 052 and 053'],
    ['RAG execution enabled', rag?.enabled === true, 'PROJECTPULSE_PULSE_AI_PRIVATE_RAG_ENABLED=true'],
    ['Private model endpoint', rag?.inferenceConfigured === true && rag?.inferenceEndpointPrivate === true, 'Private HTTPS OpenAI-compatible inference'],
    ['Private model authentication', rag?.inferenceAuthenticationConfigured === true, 'Write-only bearer secret'],
    ['Retrieval path', rag?.retrievalReady === true, rag?.hybridRetrievalReady ? 'Private embeddings + permission-scoped index' : 'Explicitly approved lexical-only retrieval']
  ];
  const completed = gates.filter(([, ready]) => ready).length;
  return <section className={`celar-production-rag-activation${completed === gates.length ? ' is-ready' : ''}`} aria-labelledby="celar-private-rag-activation-heading">
    <div className="celar-production-rag-activation-heading">
      <div><p>Private RAG activation</p><h2 id="celar-private-rag-activation-heading">{completed === gates.length ? '100% configuration-ready' : `${completed} of ${gates.length} required gates ready`}</h2><span>Free self-hosted software can satisfy the service layer; infrastructure, TLS, monitoring, backups, and operations still require an approved private environment.</span></div>
      <button type="button" onClick={openKnowledge}>Open Knowledge &amp; RAG</button>
    </div>
    <div className="celar-production-rag-gates">{gates.map(([label, ready, detail]) => <article key={label} className={ready ? 'is-ready' : 'is-blocked'}><span aria-hidden="true">{ready ? '✓' : '!'}</span><div><strong>{label}</strong><small>{detail}</small></div></article>)}</div>
    {asArray(rag?.blockers).length ? <details><summary>Remaining configuration blockers</summary><ul>{asArray(rag.blockers).map((blocker) => <li key={blocker}>{blocker}</li>)}</ul></details> : null}
    <div className="celar-production-free-stack"><strong>Free self-hosted path</strong><span>ClamAV for malware scanning · Tesseract 5 behind the private OCR adapter · Ollama for OpenAI-compatible embeddings and inference · the existing PostgreSQL permission-scoped hybrid index.</span></div>
  </section>;
}
...
function TrainingWorkspace({ data, datasets, refresh, canManage, readiness }) {
  ...
  return <section className="celar-production-workspace">
    ...
    <Notice notice={notice} error={error} />
    <div className="celar-production-runtime-banner"><strong>{readiness?.configured ? title(readiness?.status || 'training readiness unknown') : 'Governed training not configured'}</strong><span>{readiness?.configured ? 'Private training endpoint configured.' : 'Optional model training requires approved private compute, a reviewed immutable dataset, evaluation, and human promotion. Automatic document indexing is configured separately.'}</span></div>
    <div className="celar-production-two-column">
...
export default function CelarAiProductionPlatform() {
  ...
  return <main className="celar-production-platform projectpulse-module-standard" data-module="011" data-authoritative-celar-ai-platform="production-v1">
    ...
    <Notice notice={notice} error={error} />
    <section className="celar-production-summary" aria-label="Celar AI production readiness"><StatusCard label="Lifecycle schema" value={lifecycle.schemaReady ? 'Ready' : title(lifecycle.status || 'Not checked')} detail={lifecycle.schemaVersion || 'Production runtime schema'} tone={lifecycle.schemaReady ? 'ready' : 'warning'} /><StatusCard label="Basic competency" value={formatPercent(competency.currentPassRate)} detail={`${competency.passed || 0}/${competency.total || 0} required intent routes`} tone={competency.currentPassRate === 1 ? 'ready' : 'warning'} /><StatusCard label="Private RAG" value={title(rag.status || 'Not checked')} detail={rag.inferenceConfigured ? (rag.retrievalReady ? 'Private model and retrieval configured' : 'Private retrieval configuration still required') : 'Private model and retrieval configuration still required'} tone={rag.status === 'private_rag_ready' ? 'ready' : 'warning'} /><StatusCard label="Private training" value={training.configured ? title(training.status || 'Ready') : 'Governed opt-in'} detail={training.configured ? 'Private training endpoint configured' : 'Optional; automatic indexing remains separate'} tone={training.status === 'private_training_route_ready' ? 'ready' : 'neutral'} /><StatusCard label="Lifecycle records" value={String(Object.values(counts).reduce((sum, value) => sum + Number(value || 0), 0))} detail="Datasets, jobs, evaluations, models, deployments, and quality evidence" tone="neutral" /></section>
    <PrivateRagActivation rag={rag} openKnowledge={() => setActiveTab('knowledge')} />
