...
export default function ProjectForgeFlowHiveSyncPortal() {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(synchronizeHostAndProject, 1000);
  ...
  async function loadSharedWorkspace(selectedProjectId = projectId, options = {}) {
    ...
    try {
      ...
      setDirty(false);
      if (!options.silent) setNotice(workingCopy ? 'The shared PM working plan is synchronized with FlowHive.' : 'No FlowHive working copy has been saved for this project yet.');
    } catch (loadError) {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => void loadSharedWorkspace(projectId, { silent: true }), 30000);
  ...
  function changeTask(wbsNumber, patch) {
    ...
    setDirty(true);
    setNotice('Unsaved shared-plan changes are pending.');
  }

  function changeTaskPhase(task, phaseWbs) {
    setPlan((current) => moveFlowHiveTask(current, task.wbsNumber, '', phaseWbs, 'after'));
    setDirty(true);
    setNotice('Task moved to a different FlowHive phase. Save the shared PM working plan to synchronize it.');
  }

  function addTask() {
    setPlan((current) => addFlowHiveTask(current, phaseForNewTask, createTask));
    setDirty(true);
    setNotice(`A new ${PHASES.find((phase) => phase.wbs === phaseForNewTask)?.name || 'project'} task was added.`);
  }
  ...
  function deleteTask(task) {
    ...
    setDirty(true);
    setNotice('The task was removed and the shared WBS was renumbered.');
  }

  function moveTask(task, offset) {
    setPlan((current) => moveFlowHiveTaskByOffset(current, task.wbsNumber, offset));
    setDirty(true);
    setNotice('Task order changed. Save the shared PM working plan to synchronize it.');
  }
  ...
  function openFlowHive() {
    if (projectId) window.sessionStorage.setItem('projectPulseFlowHiveRequestedProject', projectId);
    ...
    <section className={`forge-flowhive-sync ${expanded ? 'is-expanded' : ''}`} aria-label="Project Forge and FlowHive synchronized working plan">
      <header className="forge-flowhive-sync__header">
        ...
        <div className="forge-flowhive-sync__header-actions">
          <button type="button" onClick={() => void loadSharedWorkspace(projectId)} disabled={!projectId || loading || saving}>{loading ? 'Refreshing…' : 'Refresh shared plan'}</button>
