...
const EMPTY_SESSION = { targetKind: 'platform', targetReference: 'Pulse', incidentId: '', note: '' };
const EMPTY_REMEDIATION = { diagnosticSessionId: '', runbookCode: 'platform_health_refresh', actionCode: 'refresh_health_snapshot', targetReference: 'Pulse', justification: '' };
...
export default function SystemDiagnosticRemediationCenter({ authSession }) {
  ...
  function chooseRunbook(code) {
    ...
    setRemediationForm((form) => ({ ...form, runbookCode: code, actionCode: runbook?.actions?.[0] ?? '', targetReference: form.targetReference || 'Pulse' }));
  }
  ...
  return (
    <section className="system-diagnostic-center" data-module="998" data-contract-version={state.data.overview?.contractVersion ?? '2026-07-21.2'} data-execution-mode="governed-native">
      <header className="system-diagnostic-hero">
        <div className="system-diagnostic-brand"><img src={usSignalLogoDataUrl} alt="US Signal" /><div><p className="system-diagnostic-eyebrow">Pulse · Module 998</p><h1>System Diagnostic &amp; Controlled Remediation Center</h1><p>Run safe checks, correlate incidents and platform evidence, rank probable issues, prepare runbooks, require separate approval, execute enabled actions, and verify recovery.</p></div></div>
