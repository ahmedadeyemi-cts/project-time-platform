...
export default function PulseAiPrivateRagWorkbench() {
  ...
  const loadReadiness = useCallback(async () => { setBusy(true); setError(''); try { setReadiness(await getJson('/api/celar-ai/v1/rag/readiness')); } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'Private RAG readiness could not be loaded.'); } finally { setBusy(false); } }, []);
  ...
  async function loadAudit(event) { event.preventDefault(); setBusy(true); setError(''); setNotice(''); try { setAudit(await getJson(`/api/celar-ai/v1/rag/answers/${encodeURIComponent(answerRunId.trim())}`)); } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'Answer audit could not be loaded.'); } finally { setBusy(false); } }
  async function submitFeedback(event) { event.preventDefault(); setBusy(true); setError(''); setNotice(''); try { const payload = await postJson(`/api/celar-ai/v1/rag/answers/${encodeURIComponent(answerRunId.trim())}/feedback`, { feedbackType: feedback.feedbackType, feedbackReason: feedback.feedbackReason, requestTrainingCandidate: false }); setNotice(title(payload.status)); } catch (actionError) { setError(actionError instanceof Error ? actionError.message : 'Feedback could not be recorded.'); } finally { setBusy(false); } }
  ...
  return <section className="pulse-ai-rag-workbench" data-pulse-ai-private-rag="v1">
    ...
    <div className="pulse-ai-rag-panel"><div className="pulse-ai-rag-panel-heading"><div><p className="pulse-ai-rag-eyebrow">Private RAG workspace</p><h3>{tab.label}</h3><p>{tab.description}</p></div>{activeTab === 'readiness' ? <button type="button" onClick={loadReadiness} disabled={busy}>Refresh readiness</button> : null}</div>
      ...
      {activeTab === 'help' ? <><form className="pulse-ai-rag-form" onSubmit={(event) => { event.preventDefault(); void run('/api/celar-ai/v1/rag/help-search', help); }}><label>Question<textarea required rows="5" value={help.question} onChange={(event) => setHelp((current) => ({ ...current, question: event.target.value }))} placeholder="Ask a detailed question about Pulse, a project, authorized documents, workflows, reports, or system behavior." /></label><div className="pulse-ai-rag-form-grid"><label>Project code<input value={help.projectCode} onChange={(event) => setHelp((current) => ({ ...current, projectCode: event.target.value }))} /></label><label>Project name<input value={help.projectName} onChange={(event) => setHelp((current) => ({ ...current, projectName: event.target.value }))} /></label></div><label>Detail level<select value={help.detailLevel} onChange={(event) => setHelp((current) => ({ ...current, detailLevel: event.target.value }))}><option value="comprehensive">Comprehensive</option><option value="executive_and_detailed">Executive and detailed</option><option value="detailed">Detailed</option><option value="standard">Standard</option></select></label><button type="submit" disabled={busy}>Ask private Celar AI</button></form><ResultPanel payload={result} /></> : null}
      {activeTab === 'timesheet' ? <><form className="pulse-ai-rag-form" onSubmit={(event) => { event.preventDefault(); void run('/api/celar-ai/v1/rag/timesheet-suggestion', { ...timesheet, workDate: timesheet.workDate || null }); }}><div className="pulse-ai-rag-form-grid"><label>Project code<input required value={timesheet.projectCode} onChange={(event) => setTimesheet((current) => ({ ...current, projectCode: event.target.value }))} /></label><label>Project name<input value={timesheet.projectName} onChange={(event) => setTimesheet((current) => ({ ...current, projectName: event.target.value }))} /></label><label>Task code<input value={timesheet.taskCode} onChange={(event) => setTimesheet((current) => ({ ...current, taskCode: event.target.value }))} /></label><label>Task name<input value={timesheet.taskName} onChange={(event) => setTimesheet((current) => ({ ...current, taskName: event.target.value }))} /></label><label>Work date<input type="date" value={timesheet.workDate} onChange={(event) => setTimesheet((current) => ({ ...current, workDate: event.target.value }))} /></label><label>Row label<input value={timesheet.rowLabel} onChange={(event) => setTimesheet((current) => ({ ...current, rowLabel: event.target.value }))} /></label></div><label>Engineer rough note<textarea rows="5" value={timesheet.engineerNote} onChange={(event) => setTimesheet((current) => ({ ...current, engineerNote: event.target.value }))} placeholder="Describe the work actually performed. The SOW/GSD cannot prove unreported activity." /></label><button type="submit" disabled={busy}>Generate private suggestion</button></form><ResultPanel payload={result} /></> : null}
      {activeTab === 'flowhive' ? <><form className="pulse-ai-rag-form" onSubmit={(event) => { event.preventDefault(); void run('/api/celar-ai/v1/rag/flowhive-plan', flowHive); }}><div className="pulse-ai-rag-form-grid"><label>Project code<input required value={flowHive.projectCode} onChange={(event) => setFlowHive((current) => ({ ...current, projectCode: event.target.value }))} /></label><label>Project name<input value={flowHive.projectName} onChange={(event) => setFlowHive((current) => ({ ...current, projectName: event.target.value }))} /></label></div><label>Requested planning outcome<textarea rows="5" value={flowHive.requestedOutcome} onChange={(event) => setFlowHive((current) => ({ ...current, requestedOutcome: event.target.value }))} placeholder="Describe the project-plan outcome the PM and Engineering need to review." /></label><button type="submit" disabled={busy}>Generate private FlowHive draft</button></form><ResultPanel payload={result} /></> : null}
