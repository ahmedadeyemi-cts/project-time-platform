...
export default function PulseAiPrivateRagWorkbench() {
  ...
  return <section className="pulse-ai-rag-workbench" data-pulse-ai-private-rag="v1">
    ...
    <nav className="pulse-ai-rag-tabs" aria-label="Pulse AI private RAG workspaces">{TABS.map((item) => <button type="button" className={activeTab === item.id ? 'is-active' : ''} key={item.id} onClick={() => setActiveTab(item.id)}><strong>{item.label}</strong><span>{item.description}</span></button>)}</nav>
    <div className="pulse-ai-rag-panel"><div className="pulse-ai-rag-panel-heading"><div><p className="pulse-ai-rag-eyebrow">Private RAG workspace</p><h3>{tab.label}</h3><p>{tab.description}</p></div>{activeTab === 'readiness' ? <button type="button" onClick={loadReadiness} disabled={busy}>Refresh readiness</button> : null}</div>
      {busy ? <div className="pulse-ai-rag-loading" role="status">Running the authorized private Pulse AI request…</div> : null}{error ? <div className="pulse-ai-rag-error" role="alert">{error}</div> : null}{notice ? <div className="pulse-ai-rag-notice" role="status">{notice}</div> : null}
