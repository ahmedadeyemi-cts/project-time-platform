...
function providerReady(status) {
  return Boolean(
    status?.customerSyncSupported === true
      ...
      <section className="customer-sell-sync" aria-labelledby="customer-sell-sync-title">
        ...
        <div className="customer-sell-readiness-grid">
          ...
          <article>
            ...
            <small>{sellIsReady ? 'Ready to pull customers' : sellState.status?.customerSyncMessage || 'Configure ConnectWise SELL in Module 026'}</small>
          </article>
          <article>
        ...
        {sellState.status?.customerSyncMessage ? <p className="customer-sell-alert">{sellState.status.customerSyncMessage}</p> : null}
        {!sellIsReady ? (
          <div className="customer-sell-guidance">
            <strong>ConnectWise SELL must be ready before customer preview.</strong>
            <ol>
