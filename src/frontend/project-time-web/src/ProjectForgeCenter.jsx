...
export default function ProjectForgeCenter() {
  ...
  async function generateAiDraft() {
    ...
    try {
      ...
      if (!result || result?.retryable === true
        || normalize(result?.status) === 'project_planning_documents_processing'
        || normalize(result?.status) === 'project_planning_ai_temporarily_unavailable') {
        throw new Error('Project document processing did not complete during this request. The existing project documents remain queued and can be resumed without another upload.');
      }
  ...
  function renderTab() {
    switch (activeTab) {
      ...
      case 'monthly-calendar': return <CalendarMonth tasks={projectTasks} holidays={holidays} canManage={canEditWorkspace} onOpenTask={setSelectedTask} onMoveSchedule={moveSchedule} />;
      ...
      case 'decision-matrix': return <DecisionMatrix tasks={projectTasks} canManage={canEditWorkspace} onOpenTask={setSelectedTask} onMoveDecision={moveDecision} />;
      ...
      case 'gantt-chart': return <GanttChart tasks={projectTasks} dependencies={dependencies} canManage={canEditWorkspace} onOpenTask={setSelectedTask} onMoveSchedule={moveSchedule} />;
      default: return null;
    }
  }
