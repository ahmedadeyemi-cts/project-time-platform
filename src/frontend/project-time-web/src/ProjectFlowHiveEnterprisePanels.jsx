...
export function FlowHiveSaveBar({ dirty, workingCopy, canManage, busy, onSaveWorkingCopy, onSaveVersion }) {
  return <div className={`flowhive-save-bar ${dirty ? 'dirty' : 'saved'}`} role="status">
    ...
    <div className="flowhive-save-bar-actions">
      ...
      <button type="button" disabled={!canAdministerPlanner || busy} onClick={onSaveVersion}>{busy === 'save' ? 'Saving version…' : 'Save immutable version'}</button>
    </div>
  </div>;
}
  ...
  return <div className="flowhive-view-panel">
    ...
    <section className="flowhive-enterprise-card">
      ...
      <div className="flowhive-control-grid">
        <label>Contract type<select value={controls.contractType || 'unknown'} disabled={!canAdministerPlanner} onChange={(event) => setControls({ ...controls, contractType: event.target.value })}><option value="unknown">Unknown / not confirmed</option><option value="fixed_price">Fixed Price</option><option value="time_and_materials">Time and Materials</option><option value="hybrid">Hybrid</option><option value="internal">Internal</option><option value="not_billable">Not billable</option></select></label>
        <label>Currency<input maxLength="3" value={currency} disabled={!canAdministerPlanner} onChange={(event) => setControls({ ...controls, currencyCode: event.target.value.toUpperCase() })} /></label>
        <label>Approved budget<input type="number" min="0" step="0.01" value={controls.approvedBudget ?? ''} disabled={!canAdministerPlanner} onChange={(event) => setControls({ ...controls, approvedBudget: event.target.value === '' ? null : Number(event.target.value) })} /></label>
        <label>Expense budget<input type="number" min="0" step="0.01" value={controls.expenseBudget ?? ''} disabled={!canAdministerPlanner} onChange={(event) => setControls({ ...controls, expenseBudget: event.target.value === '' ? null : Number(event.target.value) })} /></label>
        <label>Contingency budget<input type="number" min="0" step="0.01" value={controls.contingencyBudget ?? ''} disabled={!canAdministerPlanner} onChange={(event) => setControls({ ...controls, contingencyBudget: event.target.value === '' ? null : Number(event.target.value) })} /></label>
        <label>PM forecast at completion<input type="number" min="0" step="0.01" value={controls.forecastAtCompletion ?? ''} disabled={!canAdministerPlanner} onChange={(event) => setControls({ ...controls, forecastAtCompletion: event.target.value === '' ? null : Number(event.target.value) })} /></label>
        <label>Percent complete method<select value={controls.percentCompleteMethod || 'task_weighted'} disabled={!canAdministerPlanner} onChange={(event) => setControls({ ...controls, percentCompleteMethod: event.target.value })}><option value="task_weighted">Task weighted</option><option value="effort_weighted">Effort weighted</option><option value="manual">PM manual</option><option value="earned_value">Earned value</option></select></label>
        <label>Status report cadence<select value={controls.statusReportCadence || 'weekly'} disabled={!canAdministerPlanner} onChange={(event) => setControls({ ...controls, statusReportCadence: event.target.value })}><option value="weekly">Weekly</option><option value="biweekly">Biweekly</option><option value="monthly">Monthly</option><option value="milestone">By milestone</option><option value="manual">Manual</option></select></label>
      </div>
      <label className="flowhive-full-width">Financial notes<textarea rows="4" value={controls.financialNotes || ''} disabled={!canAdministerPlanner} onChange={(event) => setControls({ ...controls, financialNotes: event.target.value })} placeholder="Document approved assumptions, travel expectations, exclusions, and forecast rationale." /></label>
      <footer><button type="button" className="primary" disabled={!canAdministerPlanner || busy} onClick={onSave}>{busy === 'controls' ? 'Saving…' : 'Save financial controls'}</button></footer>
    </section>
  </div>;
}
...
export function FlowHiveCustomerSharingPanel({ enterprise, controls, savedPlans, draftPlan, latestShareUrl, setLatestShareUrl, shareDraft, setShareDraft, canManage, busy, onEnableSharing, onCreateShare, onRevoke }) {
  ...
  return <article className={`flowhive-sharing-card ${controls.customerSharingEnabled ? 'enabled' : 'locked'}`}>
    ...
    {!controls.customerSharingEnabled ? <button type="button" disabled={!canAdministerPlanner || busy} onClick={onEnableSharing}>Enable customer sharing for this project</button> : <>
