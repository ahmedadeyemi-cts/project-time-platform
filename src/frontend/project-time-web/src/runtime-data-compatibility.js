...
const DIRECT_ROLE_POLICY_MARKER = 'projectpulse-role-policy-direct-fetch-v2';
...
function rolePolicyModuleNumber(pathname) {
  ...
  if (pathname.endsWith('/summary') || pathname.endsWith('/versions') || pathname.includes('/role-policy/roles/')) return '012';
  const route = String(window.location.hash || '').replace(/^#/, '').split('?')[0];
  if (route === 'roles-permissions-matrix') return '037';
  if (route === 'role-admin') return '012';
  return '';
}
...
function storedSession() {
  try {
    ...
    if (!token || (session?.expiresAt && Date.now() >= Date.parse(session.expiresAt))) {
...
async function fetchRolePolicyCandidate(previousFetch, path, init, session, moduleNumber, collections) {
...
async function directRolePolicyResponse(previousFetch, originalUrl, init) {
  ...
  const requestedPath = originalUrl.pathname;
  const legacyPath = RUNTIME_TO_LEGACY_PATH[requestedPath] || requestedPath;
  const runtimePath = LEGACY_TO_RUNTIME_PATH[legacyPath] || requestedPath;
  const collections = expectedCollections(requestedPath).length
    ? expectedCollections(requestedPath)
    : expectedCollections(legacyPath);
  const moduleNumber = rolePolicyModuleNumber(requestedPath);
  const attempts = [];

  for (const path of [...new Set([legacyPath, runtimePath])]) {
    const candidate = await fetchRolePolicyCandidate(previousFetch, `${path}${originalUrl.search}`, init, session, moduleNumber, collections);
    attempts.push({ path, status: candidate.response.status, responseKeys: Object.keys(candidate.payload || {}) });
...
if (typeof window !== 'undefined' && typeof window.fetch === 'function' && !window[MARKER]) {
  ...
  window.fetch = async function projectPulseRuntimeDataCompatibility(input, init = {}) {
    ...
    const isRolePolicy = Boolean(
      ...
      || /^\/api\/(?:runtime\/)?role-policy\/roles\/[^/]+$/.test(originalUrl.pathname)
    );
