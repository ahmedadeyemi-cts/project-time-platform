...
const read = (target) => {
  if (!fs.existsSync(target)) {
    throw new Error(`Analytics Center file is missing: ${path.relative(repoRoot, target)}`);
  }
...
for (const marker of [
  ...
  'Preview report',
  'Run & save',
  'Actual analytics results',
  'Analytics run history',
  'US Signal PDF',
  'Excel',
  'Recurring US Signal delivery',
...
for (const forbidden of [
  'Fiscal Period',
  '030Q Reporting Readiness Closeout',
  'Build Export Layout',
  'Save Report Definition Preview',
  'selectedEngineerSummaryText',
  '/api/reports/030/filter-options',
  'Validate 030 Readiness',
...
for (const marker of [
  ...
  '.analytics-result-table-wrap',
  '.analytics-coverage-footer',
  '@media print'
]) {
...
assert(
  packageJson.scripts?.['validate:analytics-center'] === 'node ./scripts/validate-analytics-center.mjs',
  'Analytics package validator is not authoritative.'
);
...
if (fullRepositoryContext) {
  ...
  for (const marker of [
    ...
    'TeamIds',
    'ContractTypes',
    'AnalyticsScheduleUpsertRequest',
    'AnalyticsScheduleDeliveryEvidence',
    'Module065MailAttachment'
  ]) {
  ...
  for (const marker of [
    ...
    'Modules 055C/055D contract type',
    'Fixed Price',
    'Time and Material'
  ]) {
  ...
  for (const marker of [
    ...
    'BuildScheduledReportAsync',
    'individualizedRecipientScope',
    'ExportUrls'
  ]) {
  ...
  for (const marker of [
    ...
    'TryAcquireSchedulerLockAsync',
    'LoadRecipientOptionsAsync',
    'UpsertActivityAsync'
  ]) {
    contains(scheduleRepository, marker, 'Analytics scheduling repository');
  }

  for (const marker of [
    'individualized branded report',
    'Module 065',
    'production_governed',
    'recipients.Length > 1',
    'scopeUserId',
    'InsertDeliveryEvidenceAsync'
  ]) {
    contains(scheduleService, marker, 'Analytics governed scheduling');
  }
  ...
  for (const marker of [
    ...
    contains(`${scheduler}\n${scheduleRepository}`, marker, 'bounded multi-replica schedule runner');
  }
  ...
  for (const marker of [
    ...
    'US Signal · ProjectPulse Analytics Center',
    'SHA256.HashData'
  ]) {
...
assert(
  count(generatedApp, "import AnalyticsCenter from './AnalyticsCenter.jsx';") === 1,
  'Generated App must import Analytics Center once.'
);
assert(
  count(generatedApp, '<AnalyticsCenter authSession={authSession} />') === 1,
  'Generated App must mount Analytics Center once.'
);
assert(
  !generatedApp.includes('<EnterpriseReportingCenter authSession={authSession} />'),
  'Former Enterprise Reporting mount remains.'
);
assert(
  !generatedApp.includes('<FinancialOperationsRecoveryWorkspace mode="reporting" authSession={authSession} />'),
  'Legacy Financial Report Center mount remains.'
);
