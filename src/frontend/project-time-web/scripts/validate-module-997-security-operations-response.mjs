...
assertInvariant(
  'MODULE_997_SEQUENTIAL_998_BOUNDARY',
  overlap.includes('draft PR 26')
    && overlap.includes('does not import, compile, call, or claim ownership')
    && overlap.includes('deployed Module 998')
    && program.includes('MapSystemDiagnosticRemediationEndpoints')
    && app.includes('SystemDiagnosticRemediationCenter')
    && !backend.includes('SystemDiagnosticRemediation')
    && !frontend.includes('SystemDiagnosticRemediation'),
  'Module 997 remains source-independent while preserving deployed Module 998'
);
