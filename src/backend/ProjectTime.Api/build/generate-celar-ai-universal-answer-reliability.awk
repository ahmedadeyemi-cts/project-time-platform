# Generates compiler copies that integrate the universal answer reliability
# service into the existing Celar AI composition root and v2 chat route.
# Usage:
...
END {
  if (mode == "services") {
    if (inserted_service != 1) {
      print "CELAR_UAR_GENERATOR_ERROR=service_registration_count:" inserted_service > "/dev/stderr"
      exit 42
    }
