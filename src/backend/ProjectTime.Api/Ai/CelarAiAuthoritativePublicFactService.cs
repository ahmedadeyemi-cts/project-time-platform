...
public sealed class CelarAiAuthoritativePublicFactService
{
    ...
    private static readonly Regex PresidentName = new(
        @"\bPresident\s+(?<name>(?:[A-Z][A-Za-z'’.-]+|[A-Z]\.)(?:\s+(?:[A-Z][A-Za-z'’.-]+|[A-Z]\.)){1,4})",
        RegexOptions.CultureInvariant | RegexOptions.Compiled);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (names.Length == 0) return FailClosed(result, "official_source_claim_not_found");
        if (names.Length > 1)
            return Conflict(result, [page], $"The official source returned conflicting president names: {string.Join(", ", names)}.");

        var conclusion = $"The current President of the United States is {names[0]}. [1]";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (names.Length == 0) return FailClosed(result, "official_source_monarch_not_found");
        if (names.Length > 1)
            return Conflict(result, [governing, king], $"The official sources returned conflicting monarch names: {string.Join(", ", names)}.");

        var conclusion = $"Jordan does not have a president. Its constitutional system is a hereditary monarchy, the King is the head of state, and the current monarch is King {names[0]}. [1] [2]";
