...
public static class EntraSecretExpirationGovernanceModule
{
        ...
        try
        {
            ...
            await using (var recipients = new NpgsqlCommand("""
                ...
                WHERE app_user.is_active = TRUE
                  AND COALESCE(app_user.login_enabled, TRUE) = TRUE
    ...
    private static async Task<IResult> AcknowledgeAsync(HttpContext context)
    {
        ...
        try
        {
            ...
            await using (var active = new NpgsqlCommand("""
                ...
                """, connection, transaction))
            {
                profileId = await active.ExecuteScalarAsync(context.RequestAborted) as Guid?;
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var skippedAcknowledged = recipients.Length - pending.Length;
        ...
        foreach (var recipient in pending)
        {
            ...
            var textBody = $"""
                ...
                Expiration: {profile.ExpiresAt:O}
                Status: {daysText}
            ...
            var htmlBody = $"""
                ...
                <ul>
                  ...
                  <li>Expiration: {Escape(profile.ExpiresAt.ToString("O"))}</li>
                </ul>
    ...
    {
        var now = DateTimeOffset.UtcNow;
        var days = (int)Math.Floor((profile.ExpiresAt - now).TotalDays);
        var warningStartsAt = profile.ExpiresAt.AddDays(-profile.CriticalStartDays);
    ...
    private static void LogSanitized(IServiceProvider services, Exception exception, string operation)
    {
        services.GetRequiredService<ILoggerFactory>()
            ...
            .LogWarning("Module 065 could not {Operation}; diagnostic type {DiagnosticType}; raw detail suppressed.",
                operation,
