...
public static class WorkRegisterSellImportModule
{
        ...
        IHttpClientFactory httpClientFactory)
    {
        ...
        if (string.IsNullOrWhiteSpace(provider.ImportMappingJson) || provider.ImportMappingJson.Trim() == "{}")
            return Invalid("ConnectWise SELL quote and pricing mappings must be verified in Module 026 before importing work.");
        ...
        if (!Uri.TryCreate(lookupValue, UriKind.Absolute, out var lookupUri)
            || !ConnectWiseSellContract.IsEndpoint(lookupUri)
        ...
        try
        {
            ...
            if (provider.AuthModel != "api_key") return Invalid("ConnectWise SELL requires its API-key connection in Module 026.");
            ...
            if (string.IsNullOrWhiteSpace(apiKey) || !ConnectWiseSellContract.TryAuthorize(outbound, apiKey))
                return Invalid("Save all three ConnectWise SELL API credential fields in Module 026 before importing.");
