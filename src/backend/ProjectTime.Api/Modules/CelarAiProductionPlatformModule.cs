...
public static partial class CelarAiProductionPlatformModule
{
    ...
    private static async Task<TrainingSubmission> SubmitTrainingAsync(IHttpClientFactory clients, Guid jobId, string method, string baseModel, DatasetRow dataset, object configuration, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var response = await clients.CreateClient("PulseAiPrivateTraining").SendAsync(request, HttpCompletionOption.ResponseHeadersRead, cancellationToken); if (!response.IsSuccessStatusCode) return new("submission_failed", string.Empty, $"private_training_http_{(int)response.StatusCode}", false);
