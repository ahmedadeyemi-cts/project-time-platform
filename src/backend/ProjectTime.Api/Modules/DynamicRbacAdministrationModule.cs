...
public static partial class ScopedRolePolicyModule
{
        ...
        HttpContext context)
    {
        ...
        if (reason is null) return ReasonRequired("assign a role");
    ...
    {
        var reason = CleanRequiredReason(request.Reason);
        if (reason is null) return ReasonRequired("remove a role");
        ...
        HttpContext context)
    {
        ...
        if (reason is null) return ReasonRequired("register or restore a module");
        ...
        HttpContext context)
    {
        ...
        if (reason is null) return ReasonRequired("retire a module");
        ...
        HttpContext context)
    {
        ...
        var reason = CleanRequiredReason(request.Reason);
        if (normalized is null) return Results.BadRequest(new { status = "invalid_module_code" });
        if (reason is null) return ReasonRequired("restore a module");
    ...
    private static IResult ReasonRequired(string operation) => Results.BadRequest(new
    {
