...
public static partial class ProjectForgeModule
{
    ...
    {
        app.MapGet("/api/project-forge/bootstrap", (Func<Guid?, Guid?, HttpContext, CancellationToken, Task<IResult>>)GetBootstrapAsync);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var projects = await ReadJsonRowsAsync(connection, ProjectsSql, command =>
            {
                ...
                AddNullableUuid(command, "project_filter", projectId);
            }, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        const string updateSql = """
            ...
            WHERE plan_id=@plan_id AND project_id=@project_id AND plan_status IN ('draft','in_review','changes_requested')
            """;
        await using (var command = new NpgsqlCommand(updateSql, connection, transaction))
        {
            ...
            if (await command.ExecuteNonQueryAsync(cancellationToken) != 1) return Results.NotFound(new { status = "editable_plan_not_found" });
        }
        await ReplacePlanRowsAsync(connection, transaction, planId, request.Tasks!, request.Dependencies ?? [], access.ActualUserId, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using (var command = new NpgsqlCommand(sql, connection, transaction))
        {
            ...
            command.Parameters.AddWithValue("actor", access.ActualUserId);
            command.Parameters.AddWithValue("plan_id", planId);
            var ids = (request.PlanTaskIds ?? []).Distinct().ToArray();
            command.Parameters.AddWithValue("all_tasks", ids.Length == 0);
            command.Parameters.AddWithValue("task_ids", ids);
        ...
        await InsertNotificationAsync(connection, transaction, ProjectForgePolicy.ReviewAssignedPolicy, plan.Value.ProjectId, request.ReviewerUserId,
            $"review:{planId}:{request.ReviewerUserId}:{RevisionKey(changed.Select(row => row.Id))}",
        ...
        await transaction.CommitAsync(cancellationToken);
        return Results.Ok(new { module = "033", status = "engineering_review_assigned", planId, request.ReviewerUserId, taskCount = changed.Count, stateChanged = true });
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        const string sql = """
            UPDATE project_forge_plan_tasks
            SET estimated_hours=@hours, hourly_rate=@rate, material_units=@material_units,
                material_unit_cost=@material_cost, fixed_cost=@fixed, travel_cost=@travel,
                equipment_cost=@equipment, miscellaneous_cost=@misc,
        ...
        await using (var command = new NpgsqlCommand(sql, connection, transaction))
        {
            ...
            AddNullableDate(command, "start_date", request.StartDate);
            AddNullableDate(command, "end_date", request.DueDate);
            command.Parameters.AddWithValue("actor", access.ActualUserId);
        ...
        await MarkReviewCompleteAsync(connection, transaction, changedPlanId, planTaskId, access.EffectiveUserId, request.ReviewNote, cancellationToken);
        await InsertAuditAsync(connection, transaction, task.Value.ProjectId, changedPlanId, planTaskId, "ESTIMATE_UPDATED", access,
            new { request.EstimatedHours, request.HourlyRate, request.ReviewNote, version }, cancellationToken);
        await InsertNotificationAsync(connection, transaction, ProjectForgePolicy.TaskUpdatedPolicy, task.Value.ProjectId, access.EffectiveUserId,
            $"estimate:{planTaskId}:v{version}", new { planId = changedPlanId, planTaskId, taskName = task.Value.TaskName, version, updatedByName = access.DisplayName, changeSummary = "The assigned engineering estimate was reviewed and updated." }, cancellationToken);
        await transaction.CommitAsync(cancellationToken);
        return Results.Ok(new { module = "033", status = "estimate_updated", planTaskId, version, stateChanged = true });
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        const string lockSql = """
            SELECT plan_status, source_kind
        ...
        const string reviewSql = """
            SELECT COUNT(*) FILTER (WHERE reviewer_user_id IS NOT NULL)::int,
                   COUNT(*) FILTER (WHERE reviewer_user_id IS NOT NULL AND COALESCE(a.review_status,'pending') <> 'completed')::int,
                   COUNT(*)::int
        ...
        const string tasksSql = """
            ...
            WHERE task.plan_id=@plan_id AND task.canonical_task_id IS NULL
        ...
        foreach (var row in rows)
        {
            ...
            await InsertCanonicalTaskAsync(connection, transaction, canonicalTaskId, plan.Value.ProjectId, taskCode, row, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        const string sql = """
            ...
            SET planned_hours=EXCLUDED.planned_hours, review_status='assigned', assignment_notes=EXCLUDED.assignment_notes,
                assigned_by_user_id=EXCLUDED.assigned_by_user_id, completed_at=NULL, updated_at=NOW(), revision_number=project_forge_plan_assignments.revision_number+1
            """;
        await using var command = new NpgsqlCommand(sql, connection, transaction);
        command.Parameters.AddWithValue("plan_id", planId);
        command.Parameters.AddWithValue("task_id", planTaskId);
    ...
    {
        const string sql = """
            INSERT INTO project_tasks(task_id,project_id,task_code,task_name,task_description,billable,is_active)
            VALUES(@id,@project_id,@code,@name,@description,TRUE,TRUE)
            """;
        await using var command = new NpgsqlCommand(sql, connection, transaction);
        ...
        CancellationToken cancellationToken)
    {
        const string sql = """
            INSERT INTO project_forge_task_details(
                ...
                kanban_category,decision_action,planned_start_date,planned_end_date,recurrence_rule,
            ...
            VALUES(@task_id,@project_id,@source_task,@task_type,@phase,@priority,@status,@kanban,@decision,
                   @start_date,@end_date,@recurrence,@percent,@hours,@rate,@material_units,@material_cost,@fixed,
        ...
        CancellationToken cancellationToken)
    {
        const string sql = """
            INSERT INTO project_assignments(
                ...
                effective_start_date,effective_end_date,allocation_percent,assigned_hours)
            VALUES(gen_random_uuid(),@project_id,@task_id,@user_id,@actor,COALESCE(@start_date,CURRENT_DATE),@end_date,100,@hours)
    ...
    private static readonly string ProjectsSql = ScopeCte + """
        SELECT to_jsonb(row_data)::text FROM (
            SELECT p.project_id AS "projectId",p.project_code AS "projectCode",p.project_name AS "projectName",
                   ...
                   COALESCE(p.planned_total_project_cost,0) AS "plannedCost",
                   COALESCE(time_summary.actual_hours,0) AS "actualHours",COALESCE(expense_summary.expenses,0) AS "expenses",
            ...
            LEFT JOIN LATERAL (SELECT COUNT(*)::int task_count,COUNT(*) FILTER(WHERE COALESCE(d.task_status,'')='completed')::int completed_count
                               FROM project_tasks t LEFT JOIN project_forge_task_details d ON d.task_id=t.task_id
                               WHERE t.project_id=p.project_id AND t.is_active=TRUE) task_summary ON TRUE
            LEFT JOIN LATERAL (SELECT COALESCE(SUM(te.hours),0)::numeric actual_hours FROM time_entries te
                               WHERE te.project_id=p.project_id AND te.status NOT IN ('manager_declined','pm_declined')) time_summary ON TRUE
    ...
    private static readonly string TasksSql = ScopeCte + """
        SELECT to_jsonb(row_data)::text FROM (
            SELECT t.task_id AS "taskId",NULL::uuid AS "planTaskId",NULL::uuid AS "planId",t.project_id AS "projectId",t.task_code AS "taskCode",
                   ...
                   d.planned_start_date AS "startDate",d.planned_end_date AS "dueDate",0::int AS "durationWorkingDays",COALESCE(d.percent_complete,0) AS "percentComplete",
                   COALESCE(d.estimated_hours,assignment_summary.assigned_hours,0) AS "estimatedHours",
                   COALESCE(actuals.actual_hours,0) AS "actualHours",COALESCE(d.hourly_rate,0) AS "hourlyRate",
                   COALESCE(d.material_units*d.material_unit_cost+d.fixed_cost+d.travel_cost+d.equipment_cost+d.miscellaneous_cost,0) AS "nonLaborEstimate",
                   COALESCE(d.material_units,0) AS "materialUnits",COALESCE(d.material_unit_cost,0) AS "materialUnitCost",
                   COALESCE(d.fixed_cost,0) AS "fixedCost",COALESCE(d.travel_cost,0) AS "travelCost",
                   COALESCE(d.equipment_cost,0) AS "equipmentCost",COALESCE(d.miscellaneous_cost,0) AS "miscCost",
                   ...
                   assignment_summary.assignee_names AS "assigneeName",FALSE AS "canEditEstimate",1 AS "revision"
            ...
            LEFT JOIN LATERAL(SELECT COALESCE(SUM(pa.assigned_hours),0)::numeric assigned_hours,
                              ...
                              FROM project_assignments pa JOIN app_users assignee ON assignee.user_id=pa.user_id WHERE pa.task_id=t.task_id) assignment_summary ON TRUE
            ...
            WHERE t.is_active=TRUE AND (@can_view_all_tasks OR EXISTS(SELECT 1 FROM project_assignments own WHERE own.task_id=t.task_id AND own.user_id=@effective_user_id))
            UNION ALL
            SELECT NULL::uuid,pt.plan_task_id,pt.plan_id,pt.project_id,pt.wbs_code,pt.task_name,pt.task_description,'review_plan',pt.task_type,pt.phase_name,
                   ...
                   COALESCE(pt.duration_working_days,0),pt.percent_complete,pt.estimated_hours,0::numeric,pt.hourly_rate,
                   pt.material_units*pt.material_unit_cost+pt.fixed_cost+pt.travel_cost+pt.equipment_cost+pt.miscellaneous_cost,
                   pt.material_units,pt.material_unit_cost,pt.fixed_cost,pt.travel_cost,pt.equipment_cost,pt.miscellaneous_cost,
            ...
            WHERE pt.canonical_task_id IS NULL AND (@can_view_all_tasks OR pt.reviewer_user_id=@effective_user_id)
    ...
    private static readonly string AssignmentsSql = ScopeCte + """
        SELECT to_jsonb(row_data)::text FROM (
            ...
            WHERE @can_view_all_tasks OR pa.user_id=@effective_user_id
            UNION ALL
            ...
            WHERE @can_view_all_tasks OR pfa.user_id=@effective_user_id
        ) row_data
        """;
    ...
    private static readonly string ExpensesSql = ScopeCte + """
        SELECT to_jsonb(row_data)::text FROM (
            ...
            WHERE u.is_current=TRUE AND u.deleted_at IS NULL ORDER BY u.uploaded_at DESC LIMIT 1000
        ) row_data
        """;
    ...
    private static readonly string PlansSql = ScopeCte + """
        SELECT to_jsonb(row_data)::text FROM (
            SELECT plan.plan_id AS "planId",plan.project_id AS "projectId",plan.plan_name AS "planName",plan.objective,
                   ...
                   plan.ai_citations AS "citations",plan.ai_warnings AS "warnings",plan.review_notes AS "reviewNotes",
            ...
            LEFT JOIN LATERAL(SELECT COUNT(*)::int task_count,COALESCE(SUM(pt.estimated_hours),0)::numeric estimated_hours FROM project_forge_plan_tasks pt WHERE pt.plan_id=plan.plan_id) counts ON TRUE
    ...
    private static readonly string DependenciesSql = ScopeCte + """
        SELECT to_jsonb(row_data)::text FROM (
            SELECT d.dependency_id AS "dependencyId",d.plan_id AS "planId",d.project_id AS "projectId",
                   predecessor.wbs_code AS "predecessorWbs",successor.wbs_code AS "successorWbs",
                   d.dependency_type AS "type",d.lag_working_days AS "lagWorkingDays"
    ...
    private static readonly string ActivitySql = ScopeCte + """
        SELECT to_jsonb(row_data)::text FROM (
            SELECT audit.audit_event_id AS "activityId",audit.project_id AS "projectId",audit.plan_id AS "planId",
                   ...
                   audit.event_metadata AS "metadata",audit.occurred_at AS "occurredAt"
    ...
    private static IResult MigrationRequired() => Results.Json(new { status = "migration_070_required", message = "Project Forge persistence is not available until migration 070 has been applied." }, statusCode: 503);
        ...
        bool IsActive)
    {
        ...
        public bool IsProjectManager => !IsProjectManagementLead && HasRole("PROJECT_MANAGER", "PROJECT_MANAGEMENT");
