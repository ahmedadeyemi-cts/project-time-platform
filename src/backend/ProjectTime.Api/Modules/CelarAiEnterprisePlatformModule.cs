...
public static class CelarAiEnterprisePlatformModule
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Results.Ok(new
        {
            ...
            architectureVersion = "celar-ai-private-first-architecture-v5-context-fabric",
            ...
            layers = new object[]
            {
                ...
                new { id = "private_knowledge", label = "Private content and retrieval graph", items = new[] { "SOW", "GSD", "IQS", "Email evidence", "Authoritative versions", "Sections", "Chunks", "Citations" } },
                new { id = "governed_tools", label = "Governed live-data tools", items = new[] { "Projects", "Time", "Capacity", "Financials", "APIs", "Diagnostics" } },
                new { id = "private_adapters", label = "Private adapter plane", items = new[] { "Malware scanning", "OCR", "Embeddings", "Vector and lexical indexes", "Private inference", "Private fine-tuning" } },
                new { id = "context_graph", label = "Temporal and policy context graph", items = new[] { "Evidence as-of", "Authoritative replacement time", "Permission at decision time", "Policy revision", "Eligible-target decision", "Correlation trace" } },
                new { id = "private_intelligence", label = "Primary: private Celar AI intelligence", items = new[] { "Private RAG", "Private model", "Deterministic calculations", "Solution composer", "Context-graph reasoning" } },
                new { id = "confidence", label = "Claim, confidence, freshness, and evidence assessment", items = new[] { "Coverage", "Freshness", "Conflicts", "Citation completeness", "Trust classification", "Unsupported-claim removal" } },
                new { id = "external", label = "Optional sanitized external stages", items = new[] { "DLP capsule", "Module 064 saved eligible order", "Claude or OpenAI", "Generic problem only" } },
            ...
            flow = new[]
            {
                ...
                "routing->context_graph",
                "private_knowledge->private_adapters",
                "private_adapters->private_intelligence",
                "private_knowledge->private_intelligence",
                "governed_tools->private_intelligence",
                "context_graph->private_intelligence",
            ...
            intelligence = new
            {
                ...
                policyContext = true,
                liveDecisionTraces = true,
                hiddenChainOfThoughtReturned = false,
                freshnessAndConfidenceGates = true,
                automaticProjectContextResolution = true,
                privateAdapterReadinessExposed = true,
                fineTuningPromotionRequiresEvaluationAndHumanApproval = true
            },
            access = AccessEvidence(identity.Value, access),
