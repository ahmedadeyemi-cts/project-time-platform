...
public static class Module064074NativeAdministration
{
    ...
    private static readonly IReadOnlyDictionary<string, ModuleDefinition> Definitions =
        new Dictionary<string, ModuleDefinition>(StringComparer.Ordinal)
        {
            ["011"] = new(
                "011",
                "Celar AI Architecture Component Catalog",
                "collection",
                "components",
                new[] { "SUPER_ADMINISTRATOR", "ADMINISTRATOR", "SYSTEM_ADMINISTRATOR", "GLOBAL_ADMINISTRATOR" },
                new[]
                {
                    Field("componentId", "Component ID", "text", true, help: "Stable architecture identifier; changing the display name does not require changing this ID."),
                    Field("name", "Display name", "text", true, help: "The business-friendly name shown in the Module 011 architecture."),
                    Field("layer", "Architecture layer", "select", true, new[] { "experience", "application", "data", "security", "private_runtime", "integration", "operations" }),
                    Field("architectureState", "Architecture state", "select", true, new[] { "current_live", "current_governed", "planned_opencloud", "deferred_opencloud", "optional_external", "future_scale", "retired" }),
                    Field("placement", "Planned placement", "select", true, new[] { "pulse_platform", "pulse_data_plane", "module_064_boundary", "opencloud_shared_vm", "external_provider", "future_gpu_compute" }),
                    Field("technology", "Technology or service", "text", help: "A non-secret product, adapter, engine, or service name."),
                    Field("versionOrModel", "Version or model label", "text", help: "A display label only. Runtime health and secrets remain deployment-managed."),
                    Field("configurationName", "Configuration label", "text", help: "A friendly label for the value that will be selected later; never enter a credential, token, endpoint, or connection string."),
                    Field("readinessSource", "Readiness authority", "text", help: "The Pulse module, API, or deployment evidence that proves operational readiness."),
                    Field("purpose", "Purpose", "textarea", true),
                    Field("dependsOn", "Depends on", "text", help: "Comma-separated stable component IDs."),
                    Field("ownerUserId", "Business owner", "identity"),
                    Field("includeInDiagram", "Include in architecture diagram", "checkbox"),
                    Field("notes", "Architecture notes", "textarea")
                },
                CelarAiArchitectureDocument),
            ["064"] = new(
                "064",
    ...
    private static JsonObject CelarAiArchitectureDocument() => new()
    {
        ["components"] = new JsonArray
        {
            ArchitectureComponent(
                "module-011-workspace",
                "Celar AI Module 011 Workspace",
                "experience",
                "current_live",
                "pulse_platform",
                "React and ASP.NET Core",
                "Protected Test release",
                "Module 011 workspace name",
                "/api/celar-ai/v1/platform/readiness",
                "The user-facing Celar AI workspace for governed intelligence, architecture, solution composition, private-runtime readiness, training, and evaluation.",
                string.Empty,
                "Application behavior is deployed now; optional private document-runtime capabilities remain separately readiness-gated."),
            ArchitectureComponent(
                "internal-data-intelligence",
                "Permission-Scoped Internal Data Intelligence",
                "application",
                "current_live",
                "pulse_platform",
                "Deterministic PostgreSQL and governed API resolvers",
                "Migration 080",
                "Internal intelligence contract name",
                "/api/celar-ai/v1/system/readiness",
                "Answers authorized people, project, task, assignment, and system questions without requiring OCR, embeddings, or private-model inference.",
                "pulse-postgresql,module-064-router",
                "This is the major PR #556 capability already deployed independently of the OpenCloud private document runtime."),
            ArchitectureComponent(
                "module-064-router",
                "Module 064 Governed Provider Router",
                "integration",
                "current_live",
                "module_064_boundary",
                "Celar AI capability routing",
                "Saved per-feature target order",
                "Provider and model display names",
                "/api/celar-ai/v1/provider-bridge/readiness",
                "Controls approved provider order, model selection, health, circuit breakers, usage, and sanitized external fallback.",
                "module-011-workspace",
                "Module 064 is the only external-provider boundary; this catalog never stores provider secrets."),
            ArchitectureComponent(
                "pulse-postgresql",
                "Pulse PostgreSQL Data and Evidence Plane",
                "data",
                "current_live",
                "pulse_data_plane",
                "PostgreSQL",
                ...
                "Module 011 production readiness",
                "Stores authoritative business data, component-catalog revisions, audit evidence, conversations, routing metadata, and private retrieval metadata.",
                string.Empty,
                "Authorization filters apply before records become eligible Celar AI evidence."),
            ArchitectureComponent(
                "governed-document-storage",
                "Governed Private Document Storage",
                "data",
                "current_governed",
                "pulse_data_plane",
                "Persistent private content storage",
                "Deployment-managed",
                "Private storage display name",
                "/api/celar-ai/v1/private-runtime/readiness",
                "Retains authorized source documents and durable processing evidence inside the approved private boundary.",
                "pulse-postgresql",
                "Storage availability does not prove malware scanning, OCR, embeddings, or private inference readiness."),
            ArchitectureComponent(
                ...
                "deferred_opencloud",
                "pulse_platform",
                "Containerized background worker",
                "Migration 081 not applied",
                "Document worker service name",
                "/api/celar-ai/v1/private-runtime/readiness",
                "Coordinates malware scanning, native extraction or OCR, chunking, indexing, citation readiness, and approved document-version processing.",
                "governed-document-storage,clamav,tesseract-5,ollama",
                "Activation remains deferred until the OpenCloud runtime is reachable and migration 081 is explicitly approved."),
            ArchitectureComponent(
                "opencloud-runtime-vm",
                "OpenCloud Shared Private Runtime VM",
                "private_runtime",
                "planned_opencloud",
                "opencloud_shared_vm",
                "Linux with Podman or OCI containers",
                "Test/UAT target",
                "OpenCloud runtime VM display name",
                "OpenCloud deployment evidence",
                "Hosts Ollama, Tesseract 5, and ClamAV as separate private containers for the initial cost-controlled Test/UAT topology.",
                string.Empty,
                "The Pulse web/API application and PostgreSQL database do not need to reside on this VM."),
            ArchitectureComponent(
                "ollama",
                ...
                "Private inference and embedding model names",
                "/api/celar-ai/v1/provider-bridge/readiness",
                "Provides private OpenAI-compatible inference and embeddings for restricted document-grounded workloads.",
                "opencloud-runtime-vm",
                "Start CPU-only for Test/UAT; move Ollama to dedicated GPU-capable compute later if latency or concurrency requires it."),
            ArchitectureComponent(
                "tesseract-5",
                "Tesseract 5 OCR Adapter",
                "private_runtime",
                "planned_opencloud",
                "opencloud_shared_vm",
                "Tesseract OCR",
                "5.x",
                "OCR adapter and language-pack names",
                "/api/celar-ai/v1/private-runtime/readiness",
                "Extracts text from scanned or image-only documents when native document extraction is unavailable.",
                "opencloud-runtime-vm,clamav",
                "Searchable PDFs, DOCX, PPTX, XLSX, and text sources should bypass OCR when native extraction succeeds."),
            ArchitectureComponent(
                "clamav",
                ...
                "opencloud_shared_vm",
                "ClamAV",
                "Version and signature policy to be selected",
                "Scanner service and signature-policy names",
                "/api/celar-ai/v1/private-runtime/readiness",
                "Scans every newly uploaded document before extraction, indexing, retrieval, or model use.",
                "opencloud-runtime-vm",
                "A clean per-file scan attestation is required; merely recording a scanner name never marks a document safe."),
            ArchitectureComponent(
                "openai-external",
                "OpenAI Optional External Reasoning",
                "integration",
                "optional_external",
                "external_provider",
                "OpenAI through Module 064",
                "Module 064 managed",
                "OpenAI model display name",
                "Module 064 provider health",
                "Provides eligible generic reasoning after privacy-safe de-identification when the saved capability route permits it.",
                "module-064-router",
                "Never receives raw SOW/GSD content, customer identity, people records, financial values, or private retrieved evidence."),
            ArchitectureComponent(
                "claude-external",
                "Claude Optional External Reasoning",
                "integration",
                "optional_external",
                "external_provider",
                "Claude through Module 064",
                "Module 064 managed",
            ...
            ArchitectureComponent(
                "ollama-gpu-scale",
                "Ollama Production GPU Scale-Out",
                "operations",
                "future_scale",
                "future_gpu_compute",
                "GPU-capable private compute",
                "Capacity-driven future state",
                "Production inference compute name",
                "Production capacity and latency evidence",
                "Moves only Ollama inference and embeddings to dedicated GPU-capable compute when production load justifies it.",
                "ollama",
                "Tesseract and ClamAV may remain together on CPU compute; the private service contracts do not change.")
        }
    };

    private static JsonObject ArchitectureComponent(
        string componentId,
        string name,
        string layer,
        string architectureState,
        string placement,
        string technology,
