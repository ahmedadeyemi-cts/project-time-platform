    ...
    private static async Task<IResult> VersionsAsync(HttpContext context)
    {
        ...
        while (await reader.ReadAsync())
        {
            versions.Add(new
            {
                ...
                restoredFromPolicyVersionId = reader.IsDBNull(7) ? (Guid?)null : reader.GetGuid(7),
                createdAt = reader.GetFieldValue<DateTimeOffset>(8),
                publishedAt = reader.IsDBNull(9) ? (DateTimeOffset?)null : reader.GetFieldValue<DateTimeOffset>(9),
                retiredAt = reader.IsDBNull(10) ? (DateTimeOffset?)null : reader.GetFieldValue<DateTimeOffset>(10),
