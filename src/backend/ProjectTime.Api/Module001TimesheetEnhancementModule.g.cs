...
public static partial class ScopedRolePolicyModule
{
    ...
    private static async Task<IResult> Module001WorkQueueAsync(HttpContext context)
    {
        ...
        while (await reader.ReadAsync())
        {
            tasks.Add(new
            {
                ...
                customerId = reader.IsDBNull(1) ? (Guid?)null : reader.GetGuid(1),
    ...
    private static async Task<IResult> Module001WeeklyLinesAsync(HttpContext context)
    {
        ...
        while (await reader.ReadAsync())
        {
            lines.Add(new
            {
                ...
                assignmentId = reader.IsDBNull(3) ? (Guid?)null : reader.GetGuid(3),
                nonProjectCategoryId = reader.IsDBNull(4) ? (Guid?)null : reader.GetGuid(4),
        ...
        HttpContext context)
    {
            ...
            """, connection, transaction))
        {
            ...
            oldProjectId = reader.IsDBNull(1) ? (Guid?)null : reader.GetGuid(1);
